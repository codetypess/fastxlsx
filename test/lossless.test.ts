...
test("sheet windows expose worksheet comments separately from sparse cells", () => {
    ...
    assert.equal(
        window.cells.find((cell) => cell.address === "C3"),
        undefined
    );
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetData>
    ...
    assert.deepEqual(valueWindow.cells, [
        { address: "B2", columnNumber: 2, rowNumber: 2, value: 2 },
    ]);
...
test("sheet windows refresh after structural edits", () => {
    ...
    assert.deepEqual(
        before.cells.map((cell) => cell.address),
        ["A1", "B1", "B2"]
    );
    ...
    assert.deepEqual(
        after.cells.map((cell) => cell.address),
        ["B1", "C1", "C2"]
    );
...
test("workbook supports ArrayBuffer open flows", async () => {
    ...
    const reopened = Workbook.fromArrayBuffer(
...
test("workbook sheet name APIs are case-insensitive like Excel", () => {
    ...
    assert.throws(
        () => Workbook.create({ sheets: ["Data", "data"] }),
        /Sheet already exists: data/
    );
...
test("sheet.batch groups multiple edits and flushes the final used range", async () => {
    ...
    assert.match(
        entryText(workbook.toEntries(), "xl/worksheets/sheet1.xml"),
        /<dimension ref="A1:C3"\/>/
    );
});
...
test("sheet.batch stages cell XML lazily until a full-sheet read needs a flush", async () => {
    ...
    const sheet = workbook.getSheet("Sheet1");
    const readSheetXml = () =>
        (workbook as any).readEntryText("xl/worksheets/sheet1.xml") as string;
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetData>
    ...
    assert.doesNotMatch(
        entryText(reparsed.toEntries(), "xl/worksheets/sheet1.xml"),
        /<tableParts\b/
    );
    assert.doesNotMatch(
        entryText(reparsed.toEntries(), "xl/worksheets/_rels/sheet1.xml.rels"),
        /table1\.xml/
    );
    assert.doesNotMatch(
        entryText(reparsed.toEntries(), "[Content_Types].xml"),
        /tables\/table1\.xml/
    );
});
...
test("workbook addSheet and deleteSheet tolerate single-quoted workbook metadata attributes", async () => {
    ...
    assert.deepEqual(
        reparsed.getSheets().map((sheet) => sheet.name),
        ["Sheet1"]
    );
    assert.doesNotMatch(entryText(reparsed.toEntries(), "xl/workbook.xml"), /Sheet2/);
    assert.doesNotMatch(
        entryText(reparsed.toEntries(), "xl/_rels/workbook.xml.rels"),
        /sheet2\.xml/
    );
    assert.doesNotMatch(
        entryText(reparsed.toEntries(), "[Content_Types].xml"),
...
<workbook xmlns='http://schemas.openxmlformats.org/spreadsheetml/2006/main' xmlns:r='http://schemas.openxmlformats.org/officeDocument/2006/relationships'>
  ...
  <definedNames>
    ...
    assert.match(
        workbookXml,
        /<definedName name="LocalValue" localSheetId="0">\$D\$4<\/definedName>/
    );
});
...
test("font definition APIs read, clone, and update workbook fonts", async () => {
    ...
    assert.match(
        stylesXml,
        /<font><b\/><i\/><color rgb="FFFF0000"\/><sz val="11"\/><name val="Calibri"\/><family val="2"\/><\/font>/
    );
});
...
test("number format APIs read, clone, and update workbook numFmts", async () => {
    ...
    assert.match(
        stylesXml,
        /<numFmts count="1"><numFmt numFmtId="164" formatCode="0.000"\/><\/numFmts><fonts/
    );
});
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetData>
    ...
    assert.match(
        sheetXml,
        /<row r="1">\s*<c r="A1" t="inlineStr"><is><t>Hello<\/t><\/is><\/c>\s*<\/row>/
    );
    assert.match(sheetXml, /<row r="2" s="6" customFormat="1"\/>/);
    assert.match(
        sheetXml,
        /<row r="3" s="7" customFormat="1">\s*<c r="A3"><v>3<\/v><\/c>\s*<\/row>/
    );
});
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetData>
    ...
    assert.match(
        sheetXml,
        /<row r="3" ht="30" customHeight="1">\s*<c r="A3"><v>3<\/v><\/c>\s*<\/row>/
    );
});
...
test("row and column style definition APIs read and clone style definitions", async () => {
    ...
    assert.match(
        sheetXml,
        /<cols><col min="2" max="2" style="1"\/><col min="3" max="3" style="3"\/><col min="4" max="4" style="5"\/><\/cols>/
    );
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <sheetData>
    ...
    assert.match(
        sheetXml,
        /<cols><col min="1" max="1" style="1"\/><col min="3" max="3" style="7"\/><col min="4" max="4" hidden="1"\/><\/cols>/
    );
});
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <sheetData>
    ...
    assert.match(
        sheetXml,
        /<cols><col min="2" max="2" width="24" customWidth="1"\/><col min="4" max="4" hidden="1"\/><\/cols>/
    );
});
...
test("range APIs read and write rectangular values", async () => {
    ...
    assert.deepEqual(sheet.getRange("A1:B2"), [
        ["Hello", null],
        [null, null],
    ]);
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <mergeCells count="1"><mergeCell ref="A2:B3"/></mergeCells>
    ...
    assert.match(
        sheetXml,
        /<row r="3">[\s\S]*<c r="A3"><v>2<\/v><\/c>[\s\S]*<c r="B3"><v>4<\/v><\/c>[\s\S]*<\/row>/
    );
    assert.match(
        sheetXml,
        /<row r="4">[\s\S]*<c r="A4"><f>Sheet1!A3<\/f><v>2<\/v><\/c>[\s\S]*<\/row>/
    );
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <hyperlinks><hyperlink ref="C2" location="#Sheet1!A1"/></hyperlinks>
    ...
    assert.match(
        sheetXml,
        /<filterColumn colId="2"><filters><filter val="Beta"\/><\/filters><\/filterColumn>/
    );
    ...
    assert.match(
        sheetXml,
        /<dataValidations count="1"><dataValidation type="whole" sqref="A2:C4"\/><\/dataValidations>/
    );
    assert.match(
        sheetXml,
        /<hyperlinks><hyperlink ref="D2" location="#Sheet1!A1"\/><\/hyperlinks>/
    );
    assert.match(
        workbookXml,
        /<definedName name="_xlnm.Print_Area" localSheetId="0">\$A\$1:\$D\$4<\/definedName>/
    );
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <sortState ref="A2:D3">
    ...
    assert.match(
        sheetXml,
        /<filterColumn colId="2"><filters><filter val="D"\/><\/filters><\/filterColumn>/
    );
    assert.doesNotMatch(
        sheetXml,
        /<filterColumn colId="1"><filters><filter val="B"\/><\/filters><\/filterColumn>/
    ...
    assert.match(
        sheetXml,
        /<sortState ref="A2:C3"><sortCondition ref="C2:C3" descending="1"\/><\/sortState>/
    );
});
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <mergeCells count="1"><mergeCell ref="A2:B4"/></mergeCells>
    ...
    assert.match(
        sheetXml,
        /<row r="2">[\s\S]*<c r="A2"><f>#REF!<\/f><v>2<\/v><\/c>[\s\S]*<c r="B2"><v>3<\/v><\/c>[\s\S]*<\/row>/
    );
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <hyperlinks><hyperlink ref="C3" location="#Sheet1!A1"/></hyperlinks>
    ...
    assert.match(
        sheetXml,
        /<dataValidations count="1"><dataValidation type="whole" sqref="A2:B3"\/><\/dataValidations>/
    );
    assert.match(
        sheetXml,
        /<hyperlinks><hyperlink ref="C2" location="#Sheet1!A1"\/><\/hyperlinks>/
    ...
    assert.match(
        workbookXml,
        /<definedName name="_xlnm.Print_Area" localSheetId="0">\$A\$1:\$C\$3<\/definedName>/
    );
...
<table xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" id="1" name="Sales" displayName="Sales" ref="A1:B2" totalsRowShown="0">
  ...
  <tableColumns count="2">
    ...
    tableXml = workbook.listEntries().includes("xl/tables/table1.xml")
        ? entryText(workbook.toEntries(), "xl/tables/table1.xml")
        : "";
    const sheetXml = entryText(workbook.toEntries(), "xl/worksheets/sheet1.xml");
    assert.deepEqual(sheet.getTables(), []);
...
test("row APIs read sparse rows and write from a column offset", async () => {
    ...
    assert.match(
        sheetXml,
        /<row r="4"><c r="B4" t="inlineStr"><is><t>Name<\/t><\/is><\/c><c r="C4"\/><c r="D4" t="inlineStr"><is><t>Score<\/t><\/is><\/c><\/row>/
    );
...
test("entry APIs iterate existing worksheet cells without dense null scans", async () => {
    ...
    assert.deepEqual(summarizeCellEntries(sheet.getRowEntries(2)), [
        { address: "A2", rowNumber: 2, columnNumber: 1, type: "string", value: "Name" },
        { address: "C2", rowNumber: 2, columnNumber: 3, type: "number", value: 98 },
    ]);
    assert.deepEqual(summarizeCellEntries(sheet.getPhysicalRowEntries(2)), [
        { address: "A2", rowNumber: 2, columnNumber: 1, type: "string", value: "Name" },
        { address: "B2", rowNumber: 2, columnNumber: 2, type: "blank", value: null },
        { address: "C2", rowNumber: 2, columnNumber: 3, type: "number", value: 98 },
    ]);
    assert.deepEqual(summarizeCellEntries(sheet.getColumnEntries("C")), [
        { address: "C2", rowNumber: 2, columnNumber: 3, type: "number", value: 98 },
        { address: "C4", rowNumber: 4, columnNumber: 3, type: "string", value: "Tail" },
    ]);
    assert.deepEqual(summarizeCellEntries(sheet.getPhysicalColumnEntries("C")), [
        { address: "C2", rowNumber: 2, columnNumber: 3, type: "number", value: 98 },
        { address: "C4", rowNumber: 4, columnNumber: 3, type: "string", value: "Tail" },
    ]);
...
test("append row APIs add rows at the sheet tail", async () => {
    ...
    assert.match(
        sheetXml,
        /<row r="2"><c r="B2" t="inlineStr"><is><t>Tail<\/t><\/is><\/c><c r="C2"><v>1<\/v><\/c><\/row>/
    );
    assert.match(
        sheetXml,
        /<row r="4"><c r="B4" t="inlineStr"><is><t>Tail-3<\/t><\/is><\/c><c r="C4"><v>3<\/v><\/c><\/row>/
    );
});
...
test("record APIs map rows by header cells", async () => {
    ...
    assert.match(
        sheetXml,
        /<row r="5"><c r="A5" t="inlineStr"><is><t>Cara<\/t><\/is><\/c><c r="B5"><v>91<\/v><\/c><\/row>/
    );
});
...
test("record APIs initialize headers on a blank created sheet", async () => {
    ...
    assert.match(
        sheetXml,
        /<row r="2"><c r="A2" t="inlineStr"><is><t>Alice<\/t><\/is><\/c><c r="B2"><v>98<\/v><\/c><\/row>/
    );
});
...
test("record APIs can append multiple records in order", async () => {
    ...
    assert.match(
        sheetXml,
        /<row r="3"><c r="A3" t="inlineStr"><is><t>Bob<\/t><\/is><\/c><c r="B3"><v>87<\/v><\/c><\/row>/
    );
});
...
test("record APIs can read and update a specific record row", async () => {
    ...
    assert.match(
        sheetXml,
        /<row r="2"><c r="A2" t="inlineStr"><is><t>Alicia<\/t><\/is><\/c><c r="B2"><v>99<\/v><\/c><\/row>/
    );
});
...
test("record APIs can replace the full record set", async () => {
    ...
    sheet.setRecords([{ Name: "Zoe", Score: 100 }, { Name: "Yan" }]);
    ...
    assert.match(
        sheetXml,
        /<row r="2"><c r="A2" t="inlineStr"><is><t>Zoe<\/t><\/is><\/c><c r="B2"><v>100<\/v><\/c><\/row>/
    );
    assert.match(
        sheetXml,
        /<row r="3"><c r="A3" t="inlineStr"><is><t>Yan<\/t><\/is><\/c><c r="B3"\/><\/row>/
    );
    assert.doesNotMatch(sheetXml, /<row r="4">/);
});
...
test("sheet CSV helpers import and export header-mapped records", async () => {
    ...
    assert.equal(sheet.toCsv(), 'id,name,notes\n1001,Alpha,"A, B"\n1002,Beta,"line 1\nline 2"');
});
...
test("sheet CSV helpers support export and import options", async () => {
    ...
    assert.equal(sheet.toCsv({ includeHeaders: false, lineEnding: "\r\n" }), "1001,Alpha");
});
...
test("sheet JSON and CSV helpers support append, update, and upsert semantics", async () => {
    ...
    sheet.fromJson([{ name: "Alpha", id: 1001, note: "first" }], {
...
test("sheet record workflow APIs import, export, and sync records", async () => {
    ...
    assert.equal(
        sheet.exportRecords({ format: "csv" }),
        "id,name,note\n1001,Alpha,first\n1002,Beta,second"
    );

    const appended = sheet.importRecords([{ id: 1003, name: "Gamma", note: "third" }], {
        mode: "append",
    });
    ...
    const updated = sheet.importRecords(
        [
            { id: 1001, note: "first-updated" },
            { id: 9999, note: "missing" },
        ],
        {
            keyField: "id",
            mode: "update",
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetData>
    ...
    assert.match(
        workbookXml,
        /<definedName name="LocalToSheet1" localSheetId="1">\$A\$1<\/definedName>/
    );
    assert.match(
        workbookXml,
        /<definedName name="LocalToSheet2" localSheetId="2">\$A\$1<\/definedName>/
    );
    assert.match(
        workbookXml,
        /<definedName name="LocalToSheet3" localSheetId="0">\$A\$1<\/definedName>/
    );
    assert.match(
        appXml,
        /<vt:lpstr>Sheet3<\/vt:lpstr><vt:lpstr>Sheet1<\/vt:lpstr><vt:lpstr>Sheet2<\/vt:lpstr>/
    );
});
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <sheetData>
    ...
    assert.match(
        sheetXml,
        /<pane state="frozen" xSplit="2" ySplit="1" topLeftCell="C2" activePane="bottomRight"\/>/
    );
    assert.match(
        sheetXml,
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetData>
    ...
    assert.deepEqual(
        workbook.getSheets().map((candidate) => candidate.name),
        ["Data Set", "Sheet2"]
    );
    ...
    assert.match(
        sheet2Xml,
        /<hyperlinks><hyperlink ref="A1" location="#'Data Set'!A1"\/><\/hyperlinks>/
    );
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetData>
    ...
    assert.match(
        sheetXml,
        /<hyperlinks><hyperlink ref="B2" location="#Sheet2!A1"\/><\/hyperlinks>/
    );
...
test("sheet comment APIs create, update, read, and delete comments", async () => {
    ...
    assert.match(
        commentsXml,
        /<comment ref="B2" authorId="0"><text><t>Hello comment<\/t><\/text><\/comment>/
    );
    assert.match(
        relsXml,
...
test("insertRow shifts worksheet comments and preserves author and text after reload", () => {
    ...
    assert.match(
        entryText(reopened.toEntries(), "xl/drawings/vmlDrawing1.vml"),
        /<x:Row>3<\/x:Row>/
    );
    assert.match(
        entryText(reopened.toEntries(), "xl/drawings/vmlDrawing1.vml"),
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <autoFilter ref="A1:C2">
    ...
    assert.match(
        sheetXml,
        /<filterColumn colId="0">\s*<filters blank="1"><filter val="Alpha"\/><\/filters>\s*<\/filterColumn>/
    );
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <sortState ref="B1:D2" caseSensitive="1">
    ...
    assert.match(
        sheetXml,
        /<filterColumn colId="2"><filters><filter val="Alpha"\/><\/filters><\/filterColumn>/
    );
    assert.match(sheetXml, /<extLst><ext uri="\{urn:test:autoFilter\}"\/><\/extLst>/);
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <sortState ref="A1:D4"><sortCondition ref="A1:A4"/></sortState>
    ...
    assert.match(
        sheetXml,
        /<filterColumn colId="3"><customFilters><customFilter operator="equal" val="\*foo\*"\/><\/customFilters><\/filterColumn>/
    );
    assert.match(sheetXml, /<extLst><ext uri="\{urn:test:autoFilter\}"\/><\/extLst>/);
    assert.match(
        sheetXml,
        /<sortState ref="A1:D4"><sortCondition ref="D1:D4" descending="1"\/><\/sortState>/
    );
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <autoFilter ref="A1:E2">
    ...
    assert.match(
        sheetXml,
        /<filterColumn colId="1"><colorFilter dxfId="9" cellColor="0"\/><\/filterColumn>/
    );
    assert.match(
        sheetXml,
        /<filterColumn colId="2"><dynamicFilter type="belowAverage" valIso="2026-04-28T00:00:00Z"\/><\/filterColumn>/
    );
    assert.match(
        sheetXml,
        /<filterColumn colId="3"><top10 top="1" percent="0" val="5"\/><\/filterColumn>/
    );
    assert.match(
        sheetXml,
        /<filterColumn colId="4"><iconFilter iconSet="5Arrows" iconId="4"\/><\/filterColumn>/
    );
...
<table xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" id="1" name="Sales" displayName="Sales" ref="A1:B3" totalsRowShown="0">
  ...
  <tableColumns count="2">
    ...
    assert.match(
        tableXml,
        /<filterColumn colId="1"><customFilters><customFilter operator="equal" val="\*Corp"\/><\/customFilters><\/filterColumn>/
    );
    ...
    assert.match(
        tableXml,
        /<filterColumn colId="2"><customFilters><customFilter operator="equal" val="\*Corp"\/><\/customFilters><\/filterColumn>/
    );
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetData>
    ...
    assert.match(
        sheetXml,
        /<dataValidations count="1"><dataValidation sqref="E2" type="list" showDropDown="0" promptTitle="Pick" prompt="Choose one"><formula1>&quot;Yes,No&quot;<\/formula1><\/dataValidation><\/dataValidations>/
    );
...
test("sheet sortRange supports multi-column sorting", () => {
    ...
    sheet.sortRange("A1:C5", {
        conditions: [{ columnNumber: 2 }, { columnNumber: 3, descending: true }],
        hasHeaderRow: true,
    });
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetData>
    ...
    assert.match(
        workbookXml,
        /<definedName name="NewLocal" localSheetId="0">\$D\$4<\/definedName>/
    );
...
test("sheet print area and print titles APIs manage local defined names", async () => {
    ...
    assert.deepEqual(sheet.setPrintTitles({ rows: "1:2", columns: "A:B" }), {
        columns: "$A:$B",
        rows: "$1:$2",
    });
...
function toEntryMap(entries: Array<{ path: string; data: Uint8Array }>): Map<string, string> {
