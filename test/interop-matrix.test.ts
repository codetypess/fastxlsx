...
test("real workbook mutation matrix preserves sheet metadata and untouched package parts", async () => {
    ...
    try {
        for (const testCase of cases) {
            ...
            assert.equal(
                editedSheet.getCell(testCase.cell),
                testCase.value,
                `edited value mismatch for ${testCase.name}`
            );
            assert.deepEqual(
                changedPaths,
                [targetSheetPath],
                `unexpected package diffs for ${testCase.name}`
            );
        }
    } finally {
...
function collectSheetInteropSnapshot(
    workbook: Workbook,
...
function setEntryText(
    ...
    return [...nextEntries, { path, data: encoder.encode(text) }].sort((left, right) =>
