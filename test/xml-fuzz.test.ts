...
test("deterministic XML formatting fuzz keeps roundtrip and write invariants stable", async () => {
    ...
    try {
        for (const seed of [1, 2, 3, 4, 5, 6]) {
            ...
            assert.equal(
                editedSheet.getCell("A1"),
                nextText,
                `edited value mismatch for seed ${seed}`
            );
            assert.equal(
                editedSheet.getFormula("B2"),
                "LEN(A1)",
                `formula mismatch for seed ${seed}`
            );
            assert.equal(
                editedSheet.getStyleId("A1"),
                1,
                `style preservation mismatch for seed ${seed}`
            );
            assert.equal(
...
function mutateXmlFormatting(xml: string, seed: number): string {
    return xml.replace(
        /<([A-Za-z_?:][\w:.-]*)([^<>]*?)(\/?)>/g,
        (source, tagName: string, attributesSource: string, selfClosing: string) => {
            ...
            if (attributes.length === 0) {
                ...
                return ((seed + tagName.length) & 1) === 0 ? source.replace("/>", " />") : source;
            }
            ...
            const rendered = rotated
                .map((attribute, index) => renderRawAttribute(attribute, seed, index))
                .join(joiner);
            const close =
                selfClosing === "/"
...
function renderRawAttribute(
    ...
    const quote = attribute.value.includes(preferredQuote) ? attribute.quote : preferredQuote;
