...
import { recalculateCellFormula, recalculateSheetFormulas } from "./formula/formula-recalc.js";
import {
    buildSheetIndex,
    getLocatedCell,
    parseCellSnapshot,
...
import { buildXmlElement, replaceXmlTagSource, rewriteXmlTagsByName } from "./sheet/sheet-xml.js";
import {
...
import {
    readWorkbookSheetValueWindow,
    readWorkbookSheetWindow,
...
export class Sheet {
    ...
    getColumnWidths(): Record<string, number> {
        ...
        for (const definition of parseColumnDefinitions(this.getSheetIndex().xml)) {
            ...
            for (
                let columnNumber = definition.min;
                columnNumber <= definition.max;
                columnNumber += 1
            ) {
    ...
    setStyle(
        ...
        return this.cloneStyle(
            addressOrRowNumber,
            (columnOrPatch as CellStylePatch | undefined) ?? {}
        );
    }
    ...
    setNumberFormat(
        ...
        const normalizedAddress = resolveCellAddress(
            addressOrRowNumber,
            typeof addressOrRowNumber === "number"
                ? (columnOrFormatCode as number | string)
                : undefined
        );
        const nextFormatCode =
            typeof addressOrRowNumber === "number"
                ? (formatCode ?? "")
                : (columnOrFormatCode as string);
        const currentStyleId = this.getStyleId(normalizedAddress) ?? 0;
    ...
    cloneStyle(
        ...
        const nextStyleId = this.workbook.cloneStyle(
            this.getStyleId(normalizedAddress) ?? 0,
            nextPatch
        );
        this.setStyleId(normalizedAddress, nextStyleId);
        return nextStyleId;
    ...
    getHeaders(headerRowNumber = 1): string[] {
        ...
        return this.getRow(headerRowNumber).map((value) =>
    ...
    toCsv(headerRowOrOptions: number | SheetExportRecordsOptions = 1): string {
        ...
        const rows = [
            ...
            ...this.getRecords(options.headerRow).map((record) =>
                headers.map((header) => formatCsvCellValue(record[header] ?? null))
            ),
        ];
    ...
    fromCsv(csv: string, headerRowOrOptions: number | SheetImportRecordsOptions = 1): void {
        ...
        const records = rows.slice(1).map((row) => {
            ...
            for (let index = 0; index < headers.length; index += 1) {
                ...
                record[header] = options.inferTypes
                    ? parseCsvCellValue(normalizedValue)
     ...
     */
    exportRecords(
        options: SheetExportRecordsOptions = {}
    ): Array<Record<string, CellValue>> | string {
        ...
        if (mode === "replace") {
            ...
            return {
                headers:
                    headers.length > 0
                        ? headers
                        : trimTrailingEmptyHeaderNames(this.getHeaders(headerRow)),
    ...
    setPrintTitles(options: { columns?: string | null; rows?: string | null }): SheetPrintTitles {
        ...
        const columns =
            options.columns === undefined ? this.getPrintTitles().columns : options.columns;
        const references: string[] = [];
        const result: SheetPrintTitles = {
            columns:
                columns === null || columns === undefined
                    ? null
                    : normalizePrintTitleColumnRef(columns),
        ...
        this.workbook.setDefinedName("_xlnm.Print_Titles", references.join(","), {
            scope: this.name,
        });
        return result;
    }
    ...
    *iterWindowColumns(options: SheetWindowReadOptions): IterableIterator<SheetWindowColumnEntry> {
    ...
    getDataValidation(range: string): DataValidation | null {
        ...
        return (
            this.getDataValidations().find((validation) => validation.range === normalizedRange) ??
            null
        );
    }
    ...
    }): SheetTableSummary[] | SheetTableWithAutoFilterSummary[] {
    ...
    tryGetTable(name: string): SheetTableHandle | null {
        ...
        return {
            ...
            getAutoFilterDefinition: () =>
                parseTableAutoFilterDefinition(readTableState().tableXml),
    ...
    getComments(): SheetComment[] {
        const parts = findSheetCommentParts(
    ...
    getHyperlinks(): Hyperlink[] {
        return parseSheetHyperlinks(
            this.getSheetIndex().xml,
            parseHyperlinkRelationshipTargets(this.readSheetRelationshipsXml())
        );
    }
    ...
    getHyperlink(address: string): Hyperlink | null {
        const normalizedAddress = normalizeCellAddress(address);
        return (
            this.getHyperlinks().find((hyperlink) => hyperlink.address === normalizedAddress) ??
            null
        );
    }
    ...
    removeHyperlink(address: string): void {
        ...
        const nextState = removeSheetHyperlink(
            currentSheetXml,
            currentRelationshipsXml,
            normalizedAddress
        );
    ...
    setComment(
        address: string,
        text: string,
        options: SheetCommentWriteOptions = {}
    ): SheetComment {
        const normalizedAddress = normalizeCellAddress(address);
        const currentSheetXml = this.getSheetIndex().xml;
        ...
        const previousComment =
            existingComments.find((comment) => comment.address === normalizedAddress) ?? null;
        const nextComments = [
            ...existingComments.filter((comment) => comment.address !== normalizedAddress),
            {
                ...
                author:
                    options.author ??
                    previousComment?.author ??
                    existingComments[0]?.author ??
                    "fastxlsx",
                text,
            },
        ...
        if (!commentsPath) {
            ...
            nextContentTypesXml = addContentTypeOverride(
                nextContentTypesXml,
                commentsPath,
                COMMENTS_CONTENT_TYPE
            );
        ...
        if (!vmlRelationshipId || !vmlPath) {
            vmlRelationshipId =
                parts.legacyDrawingRelationshipId ??
                getNextRelationshipIdFromXml(nextRelationshipsXml);
            ...
            nextContentTypesXml = ensureDefaultContentType(
                nextContentTypesXml,
                "vml",
                VML_CONTENT_TYPE
            );
    ...
    removeComment(address: string): void {
        ...
        const existingComments = parseCommentsXml(
            this.workbook.readEntryText(parts.commentsPath)
        ).comments;
        const nextComments = existingComments.filter(
            (comment) => comment.address !== normalizedAddress
        );
        ...
        if (nextComments.length > 0) {
            ...
            if (parts.vmlPath) {
                this.workbook.writeEntryText(
                    parts.vmlPath,
                    buildCommentsVmlDrawingXml(nextComments)
                );
            }
            return;
        }
        ...
        if (parts.commentsRelationshipId) {
            nextRelationshipsXml = removeRelationshipById(
                nextRelationshipsXml,
                parts.commentsRelationshipId
            );
        }
    ...
    setAutoFilter(range: string): void {
        const normalizedRange = normalizeRangeRef(range);
        const nextSheetXml = setWorksheetAutoFilterRangeInSheetXml(
            this.getSheetIndex().xml,
            normalizedRange
        );
    ...
    setAutoFilterDefinition(definition: AutoFilterDefinition): void {
        const nextSheetXml = setWorksheetAutoFilterDefinitionInSheetXml(
            this.getSheetIndex().xml,
            definition
        );
    ...
    setAutoFilterColumn(column: AutoFilterColumn): void {
        const nextSheetXml = setWorksheetAutoFilterColumnInSheetXml(
            this.getSheetIndex().xml,
            column
        );
    ...
    clearAutoFilterColumns(columnNumbers?: number[]): void {
        const nextSheetXml = clearWorksheetAutoFilterColumnsInSheetXml(
            this.getSheetIndex().xml,
            columnNumbers
        );
    ...
    freezePane(columnCount: number, rowCount = 0): void {
        ...
        const nextSheetXml = upsertFreezePaneInSheetXml(
            this.getSheetIndex().xml,
            columnCount,
            rowCount
        );
    ...
    removeDataValidation(range: string): void {
        const normalizedRange = normalizeSqref(range);
        const nextSheetXml = removeDataValidationFromSheetXml(
            this.getSheetIndex().xml,
            normalizedRange
        );
    ...
    addTable(
        ...
        const tableId = getNextTableId(this.workbook.listEntries(), (path) =>
        ...
        const tableXml = buildTableXml(
            ...
            tableId,
            name,
            this.getRange(normalizedRange)[0] ?? []
        );
        ...
        this.writeContentTypesXml(
            addContentTypeOverride(this.readContentTypesXml(), tablePath, TABLE_CONTENT_TYPE)
        );
    ...
    removeTable(name: string): void {
        ...
        this.writeSheetXml(
            removeTablePartsFromSheetXml(this.getSheetIndex().xml, [tableReference.relationshipId])
        );
        this.writeSheetRelationshipsXml(
            removeRelationshipById(this.readSheetRelationshipsXml(), tableReference.relationshipId)
        );
        this.writeContentTypesXml(
            removeContentTypeOverride(this.readContentTypesXml(), tableReference.path)
        );
    ...
    insertRow(rowNumber: number, count = 1): void {
        ...
        for (const sourceRowNumber of [...index.rowNumbers].sort((left, right) => right - left)) {
            ...
            const nextRowXml = transformRowXml(index.xml, row, this.name, 0, 0, rowNumber, count);
            nextSheetXml =
                nextSheetXml.slice(0, row.start) + nextRowXml + nextSheetXml.slice(row.end);
        }
        ...
        this.workbook.rewriteDefinedNamesForSheetStructure(
            this.name,
            0,
            0,
            rowNumber,
        ...
        if (comments.length > 0) {
            this.writeComments(shiftWorksheetComments(comments, 0, 0, rowNumber, count));
        }
    }
    ...
    insertColumn(column: number | string, count = 1): void {
        ...
        if (comments.length > 0) {
            this.writeComments(shiftWorksheetComments(comments, columnNumber, count, 0, 0));
        }
    }
    ...
    deleteRow(rowNumber: number, count = 1): void {
        ...
        if (comments.length > 0) {
            this.writeComments(deleteWorksheetComments(comments, 0, 0, rowNumber, count));
        }
    }
    ...
    deleteColumn(column: number | string, count = 1): void {
        ...
        for (const rowNumber of [...index.rowNumbers].sort((left, right) => right - left)) {
            ...
            const nextRowXml = deleteColumnTransform(
                index.xml,
                row,
                this.name,
                columnNumber,
        ...
        if (comments.length > 0) {
            this.writeComments(deleteWorksheetComments(comments, columnNumber, count, 0, 0));
        }
    }
    ...
    sortRange(range: string, options: SortRangeOptions): void {
        ...
        if (commentsInRange.length > 0) {
            throw new XlsxError(
                `sortRange does not yet support worksheet comments inside ${normalizedRange}`
            );
        }

        const sourceRows = sortContext.sourceRows.map((rowNumber) =>
            buildSortSourceRow(rowNumber, sortContext, (address) =>
                this.getCurrentCellWriteState(address)
            )
        );
        const sortedRows = [...sourceRows].sort((left, right) =>
            compareSortSourceRows(left, right, sortContext.conditions)
        );
        ...
        this.batch(() => {
            for (let index = 0; index < sortedRows.length; index += 1) {
                ...
                for (
                    let columnNumber = sortContext.startColumn;
                    columnNumber <= sortContext.endColumn;
                    columnNumber += 1
                ) {
                    ...
                    this.stagePendingCellMutation(targetAddress, {
                        ...
                        kind: "set",
                        snapshot: translateCellSnapshotForSort(
                            sourceCell.snapshot,
                            sourceRow.rowNumber,
                            targetRowNumber
                        ),
            ...
            nextSheetXml = rewriteWorksheetHyperlinksForSortedRows(
                nextSheetXml,
                sortContext,
                targetRowBySource
            );
            nextSheetXml = rewriteWorksheetDataValidationsForSortedRows(
                nextSheetXml,
                sortContext,
                targetRowBySource
            );
            ...
            nextSheetXml =
                worksheetAutoFilterDefinition &&
                worksheetAutoFilterDefinition.range === normalizedRange
            ...
            for (const table of tableReferences) {
                if (
                    table.metadata.range !== normalizedRange &&
                    table.autoFilter?.range !== normalizedRange
                ) {
                    continue;
                }
    ...
    setCell(
        ...
        const nextCellXml = buildValueCellXml(
            normalizedAddress,
            nextValue,
    ...
    setStyleId(
        ...
        const normalizedAddress = resolveCellAddress(
            addressOrRowNumber,
            typeof addressOrRowNumber === "number"
                ? (columnOrStyleId as number | string)
                : undefined
        );
    ...
    deleteCell(addressOrRowNumber: string | number, column?: number | string): void {
        ...
        if (row && row.cells.length === 1) {
            const nextRowXml = normalizeEmptyRowXml(
                index.xml.slice(row.start, existingCell.start) +
                    index.xml.slice(existingCell.end, row.end)
            );
            this.writeSheetXml(
                index.xml.slice(0, row.start) + nextRowXml + index.xml.slice(row.end)
            );
            return;
        }

        this.writeSheetXml(
            index.xml.slice(0, existingCell.start) + index.xml.slice(existingCell.end)
        );
    }
    ...
    setFormula(
        ...
        const normalizedAddress = resolveCellAddress(
            addressOrRowNumber,
            typeof addressOrRowNumber === "number"
                ? (columnOrFormula as number | string)
                : undefined
        );
        const currentCell = this.getCurrentCellWriteState(normalizedAddress);
        ...
        if (this.workbook.isBatching()) {
            this.stagePendingCellMutation(normalizedAddress, {
                attributesSource: extractCellAttributesSource(nextCellXml),
                kind: "set",
                snapshot: buildFormulaCellSnapshot(
                    formula,
                    cachedValue,
                    currentCell.snapshot.styleId
                ),
                xml: nextCellXml,
            });
            return;
    ...
    setRowHidden(rowNumber: number, hidden: boolean): void {
        ...
        if (!row) {
            ...
            const insertionIndex = findRowInsertionIndex(index, rowNumber);
            this.writeSheetXml(
                index.xml.slice(0, insertionIndex) + nextRowXml + index.xml.slice(insertionIndex)
            );
            return;
        }
        ...
        this.writeSheetXml(
            index.xml.slice(0, row.start) +
                buildUpdatedRowXml(index.xml, row, { hidden }) +
                index.xml.slice(row.end)
        );
    }
    ...
    setRowHeight(rowNumber: number, height: number | null): void {
        ...
        this.writeSheetXml(
            index.xml.slice(0, row.start) +
                buildUpdatedRowXml(index.xml, row, { height }) +
                index.xml.slice(row.end)
        );
    }
    ...
    addMergedRange(range: string): void {
        ...
        this.writeSheetXml(
            updateMergedRanges(this.getSheetIndex().xml, [...ranges, normalizedRange])
        );
    }
    ...
    setRow(rowNumber: number, values: CellValue[], startColumn = 1): void {
        ...
        for (let columnOffset = 0; columnOffset < values.length; columnOffset += 1) {
            this.setCell(
                makeCellAddress(rowNumber, startColumn + columnOffset),
                values[columnOffset]
            );
        }
    }
    ...
    setRecord(rowNumber: number, record: Record<string, CellValue>, headerRowNumber = 1): void {
        ...
        const headerMap = this.resolveRecordHeaderMap(
            headerRowNumber,
            [record],
    ...
    setRecords(records: Array<Record<string, CellValue>>, headerRowNumber = 1): void {
        ...
        const existingRecordRows = this.getSheetIndex().rowNumbers.filter(
            (rowNumber) =>
                rowNumber > headerRowNumber && this.getRecord(rowNumber, headerRowNumber) !== null
        );
        ...
        const rowsToDelete = existingRecordRows.filter(
            (rowNumber) => !targetRows.includes(rowNumber)
        );
    ...
    updateRecordBy(
        ...
        const resolvedHeaderRowNumber = useImplicitMatchValue
            ? (recordOrHeaderRowNumber as number)
            : headerRowNumber;
    ...
    deleteRecord(rowNumber: number, headerRowNumber = 1): void {
        ...
        const nextSheetXml =
            this.getSheetIndex().xml.slice(0, row.start) + this.getSheetIndex().xml.slice(row.end);
        this.writeSheetXml(nextSheetXml);
    }
    ...
    private readCellSnapshotByIndexes(
        ...
        const columnNumber = normalizeColumnNumber(column);
        if (this.hasPendingCellMutations) {
            const pendingCell = this.pendingCellMutations.get(
                makeCellAddress(rowNumber, columnNumber)
            );
            if (pendingCell) {
                return pendingCell.snapshot;
            }
    ...
    private resolveRecordHeaderMap(
        ...
        if (
            headerMap.size > 0 ||
            !allowHeaderInitialization ||
            !isEmptyHeaderRow(this.getRow(headerRowNumber))
        ) {
            return headerMap;
        }
    ...
    setRange(startAddress: string, values: CellValue[][]): void {
        ...
        const { rowNumber: startRow, columnNumber: startColumn } =
            splitCellAddress(normalizedStartAddress);
        ...
        for (let rowOffset = 0; rowOffset < values.length; rowOffset += 1) {
            ...
            for (let columnOffset = 0; columnOffset < row.length; columnOffset += 1) {
                this.setCell(
                    makeCellAddress(startRow + rowOffset, startColumn + columnOffset),
                    row[columnOffset]
                );
            }
        }
    ...
    private flushPendingCellMutations(): void {
        ...
        const nextSheetXml = applyPendingCellMutationsToSheetXml(
            baseIndex,
            this.pendingCellMutations.values()
        );
    ...
    private rewriteFormulaTexts(transformFormula: (formula: string) => string): void {
        ...
        const nextSheetXml = rewriteXmlTagsByName(sheetXml, "f", (formulaTag) => {
            ...
            return buildXmlElement(
                "f",
                parseAttributes(formulaTag.attributesSource),
                escapeXmlText(nextFormula)
            );
        });
    ...
    private getTableReferences(): TableReference[] {
        return listTableReferences(
            this.getSheetIndex().xml,
            this.path,
            this.workbook.listEntries(),
            (path) => this.workbook.readEntryText(path)
        );
    }
    ...
    private writeComments(nextComments: SheetComment[]): void {
        ...
        const entryPaths = this.workbook.listEntries();
        const parts = findSheetCommentParts(currentSheetXml, this.path, currentRelationshipsXml);
        const normalizedComments = [...nextComments].sort((left, right) =>
        ...
        if (normalizedComments.length === 0) {
            ...
            this.workbook.removeEntry(parts.commentsPath);
            nextContentTypesXml = removeContentTypeOverride(
                nextContentTypesXml,
                parts.commentsPath
            );
            if (parts.commentsRelationshipId) {
    ...
    private updateTableReferences(
        ...
        const nextSheetXml = removeTablePartsFromSheetXml(
            this.getSheetIndex().xml,
        ...
        this.writeSheetRelationshipsXml(
            removedRelationshipIds.reduce(
                (relationshipsXml, relationshipId) =>
                    removeRelationshipById(relationshipsXml, relationshipId),
                this.readSheetRelationshipsXml()
            )
        );
    ...
    private writeSheetXml(nextSheetXml: string): void {
        ...
        this.sheetIndex =
            normalizedSheetXml === nextSheetXml
                ? indexedSheet
...
function buildSortRangeContext(range: string, options: SortRangeOptions): SortRangeContext {
    ...
    for (const condition of options.conditions) {
        if (condition.columnNumber < startColumn || condition.columnNumber > endColumn) {
            throw new XlsxError(
                `Sort condition column ${condition.columnNumber} is outside range ${normalizedRange}`
            );
        }
    }
...
function buildSortSourceRow(
    ...
    for (
        let columnNumber = context.startColumn;
    ...
    return {
        cells,
        keySnapshots: context.conditions.map(
            (condition) =>
                cells.get(condition.columnNumber)?.snapshot ?? createMissingCellSnapshot()
        ),
...
function compareSortSourceRows(
    ...
    for (let index = 0; index < conditions.length; index += 1) {
        ...
        const comparison = compareSortSnapshots(
            left.keySnapshots[index]!,
            right.keySnapshots[index]!
        );
        if (comparison !== 0) {
...
function translateCellXmlForSort(
    ...
    nextInnerXml = rewriteXmlTagsByName(nextInnerXml, "f", (formulaTag) => {
        const formulaAttributes = parseAttributes(formulaTag.attributesSource).map(
            ([name, value]) => {
                if (name !== "ref") {
...
function rewriteRangeRefForSortedRows(
    ...
    if (
        ...
        throw new XlsxError(
            `Cannot sort ${context.range} while overlapping metadata range ${normalizedRange}`
        );
    }
    ...
    if (maxRow - minRow + 1 !== mappedRows.length) {
        throw new XlsxError(
            `Cannot sort ${context.range} because metadata range ${normalizedRange} would become non-contiguous`
        );
    }
...
function buildValueCellSnapshot(value: CellValue, styleId: number | null): CellSnapshot {
    return {
        ...
        error: null,
        formula: null,
        rawType: typeof value === "string" ? "inlineStr" : typeof value === "boolean" ? "b" : null,
        styleId,
        type:
            value === null
                ? "blank"
...
): Required<Pick<SheetExportRecordsOptions, "headerRow" | "includeHeaders" | "lineEnding">> &
    SheetExportRecordsOptions {
...
function resolveSheetImportOptions(
    ...
    Pick<SheetImportRecordsOptions, "headerRow" | "inferTypes" | "trimHeaders" | "trimValues">
> &
    SheetImportRecordsOptions {
    if (typeof headerRowOrOptions === "number") {
        return {
...
function applyPendingCellMutationsToSheetXml(
    ...
    for (const rowNumber of rowNumbers) {
        ...
        if (!row) {
            const nextRowXml = buildRowXmlFromPendingMutations(
                rowNumber,
                `r="${rowNumber}"`,
                rowMutations
            );
            ...
            nextSheetXml =
                nextSheetXml.slice(0, insertionIndex) +
                nextRowXml +
                nextSheetXml.slice(insertionIndex);
            continue;
        }
...
function applyPendingMutationsToRow(
    ...
    if (row.selfClosing) {
        return (
            buildRowXmlFromPendingMutations(row.rowNumber, row.attributesSource, rowMutations) ??
            sheetXml.slice(row.start, row.end)
        );
    }

    let nextRowXml = sheetXml.slice(row.start, row.end);
    const sortedMutations = [...rowMutations].sort(
        (left, right) => right.columnNumber - left.columnNumber
    );
    ...
    for (const mutation of sortedMutations) {
        ...
        if (existingCell) {
            ...
            nextRowXml =
                mutation.kind === "delete"
                    ...
                    : nextRowXml.slice(0, relativeStart) +
                      mutation.xml! +
                      nextRowXml.slice(relativeEnd);
            continue;
        }
        ...
        const insertionIndex =
            findPendingCellInsertionIndex(row, mutation.columnNumber) - row.start;
        nextRowXml =
            nextRowXml.slice(0, insertionIndex) + mutation.xml! + nextRowXml.slice(insertionIndex);
    }
...
function buildRowXmlFromPendingMutations(
    ...
    const attributesSource =
        rowAttributesSource.length > 0 ? rowAttributesSource : `r="${rowNumber}"`;
    return `<row ${attributesSource}>${nextCells.join("")}</row>`;
}

function findPendingCellInsertionIndex(
    row: Pick<LocatedRow, "cells" | "innerEnd">,
    columnNumber: number
): number {
    for (const cell of row.cells) {
