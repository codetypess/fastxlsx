...
export class Workbook {
    ...
    static create(sheetNameOrOptions: string | WorkbookCreateOptions = "Sheet1"): Workbook {
        ...
        workbook.batch((currentWorkbook) => {
            ...
            for (const sheetConfig of normalizedSheets) {
                ...
                if (sheetConfig.headers && sheetConfig.headers.length > 0) {
                    ...
                    if (sheetConfig.headerStyle) {
                        sheet.setRangeStyle(
                            `A1:${buildHeaderStyleEndAddress(sheetConfig.headers.length)}`,
                            sheetConfig.headerStyle
                        );
                    }
                }
                ...
                if (sheetConfig.frozenPane) {
                    sheet.freezePane(
                        sheetConfig.frozenPane.columnCount,
                        sheetConfig.frozenPane.rowCount ?? 0
                    );
                }
                ...
                if (sheetConfig.comments) {
                    for (const comment of sheetConfig.comments) {
                        sheet.setComment(comment.address, comment.text, {
            ...
            const activeSheetName =
                options.activeSheet ??
                normalizedSheets.find((sheet) => sheet.visibility === "visible")!.name;
    ...
    getActiveSheet(): Sheet {
        ...
        const activeSheetIndex = parseActiveSheetIndex(
            this.readEntryText(context.workbookPath),
    ...
    getStyle(styleId: number): CellStyleDefinition | null {
        ...
        return cloneCellStyleDefinition(
            this.getStylesCache()?.cellXfs[styleId]?.definition ?? null
        );
    }
    ...
    getBorder(borderId: number): CellBorderDefinition | null {
        ...
        return cloneCellBorderDefinition(
            this.getStylesCache()?.borders[borderId]?.definition ?? null
        );
    }
    ...
    updateNumberFormat(numFmtId: number, formatCode: string): void {
        ...
        this.writeEntryText(
            styles.path,
            upsertNumberFormatInStylesXml(styles.xml, numFmtId, formatCode)
        );
    }
    ...
    cloneNumberFormat(numFmtId: number, formatCode?: string): number {
        ...
        const sourceCode =
            formatCode ??
            styles.numberFormats.get(numFmtId) ??
            BUILTIN_NUMBER_FORMATS.get(numFmtId);
        ...
        this.writeEntryText(
            styles.path,
            upsertNumberFormatInStylesXml(styles.xml, nextNumFmtId, sourceCode)
        );
        return nextNumFmtId;
    }
    ...
    ensureNumberFormat(formatCode: string): number {
        ...
        this.writeEntryText(
            styles.path,
            upsertNumberFormatInStylesXml(styles.xml, nextNumFmtId, formatCode)
        );
        return nextNumFmtId;
    }
    ...
    updateBorder(borderId: number, patch: CellBorderPatch): void {
        ...
        this.writeEntryText(
            ...
            replaceBorderInStylesXml(
                styles.xml,
                borderId,
                buildPatchedBorderXml(sourceBorder, patch)
            )
        );
    }
    ...
    setSheetVisibility(sheetName: string, visibility: SheetVisibility): void {
        ...
        const visibleSheetCount = countVisibleSheets(
            workbookXml,
            context.sheets,
            parseSheetVisibility
        );
    ...
    getDefinedName(name: string, scope?: string): string | null {
        const normalizedScope =
            scope === undefined
                ? null
                : (findSheetByName(this.getWorkbookContext().sheets, scope)?.name ?? scope);
    ...
    setDefinedName(name: string, value: string, options: SetDefinedNameOptions = {}): void {
        ...
        const replacement = rewriteDefinedNamesInWorkbookXml(workbookXml, (tag) => {
            ...
            const candidateScope =
                candidateLocalSheetId === undefined ? null : Number(candidateLocalSheetId);

            return candidateName === name && candidateScope === localSheetId
                ? nextDefinedNameXml
                : tag.source;
        });
        ...
        this.writeEntryText(
            workbookPath,
            insertDefinedNameIntoWorkbookXml(workbookXml, nextDefinedNameXml)
        );
    }
    ...
    renameSheet(currentSheetName: string, nextSheetName: string): Sheet {
        ...
        for (const sheet of context.sheets) {
            ...
            this.rewriteSheetHyperlinkLocations(
                sheet.path,
                currentResolvedSheetName,
                nextSheetName
            );
        }
        ...
        this.rewriteAppSheetNames(
            context.sheets.map((sheet) =>
                sheet.relationshipId === renamedSheet.relationshipId ? nextSheetName : sheet.name
            )
        );
    ...
    deleteSheet(sheetName: string): void {
        ...
        this.rewriteAppSheetNames(
            context.sheets
                .filter((sheet) => sheet.relationshipId !== deletedSheet.relationshipId)
    ...
    rewriteDefinedNamesForSheetStructure(
        ...
        const replacement = rewriteDefinedNamesInWorkbookXml(workbookXml, (tag) => {
            ...
            return nextNameText === nameText
                ? tag.source
                : buildDefinedNameTagSource(tag.attributesSource, nextNameText);
        });
    ...
    private rewriteSheetHyperlinkLocations(
        ...
        const result = rewriteHyperlinkLocationXml(
            this.readEntryText(path),
            currentSheetName,
            nextSheetName
        );
    ...
    removeEntry(path: string): void {
        ...
        if (
            shouldResetWorkbookContext(this.workbookContext, path) ||
            ...
            typeof sheet === "string"
                ...
                : {
                      ...
                      comments: sheet.comments
                          ? sheet.comments.map((comment) => ({ ...comment }))
                          : undefined,
                      ...
                      rangeStyles: sheet.rangeStyles
                          ? sheet.rangeStyles.map((rangeStyle) => ({
                                ...rangeStyle,
                                patch: rangeStyle.patch ? { ...rangeStyle.patch } : undefined,
                            }))
                          : undefined,
        ...
        if (
            ...
            normalizedInput.some(
                (candidate, candidateIndex) =>
                    ...
                    sheetNamesEqual(resolveCreateSheetName(candidate), normalizedSheet.name)
            )
        ) {
