...
export const VML_CONTENT_TYPE = "application/vnd.openxmlformats-officedocument.vmlDrawing";
...
export function buildCommentsXml(comments: SheetComment[]): string {
    const normalizedComments = [...comments].sort((left, right) =>
    ...
    const authorsXml = authors
        .map((author) => `<author>${escapeXmlText(author)}</author>`)
        .join("");
...
export function findSheetCommentParts(
    sheetXml: string,
    sheetPath: string,
    relationshipsXml: string
): SheetCommentParts {
    const relationshipsPathDir = dirnamePosix(sheetPath);
    const commentsRelationship = findRelationshipByType(
        relationshipsXml,
        COMMENTS_RELATIONSHIP_TYPE
    );
    const legacyDrawingTag = findFirstXmlTag(sheetXml, "legacyDrawing");
    const legacyDrawingRelationshipId = legacyDrawingTag
        ? (getTagAttr(legacyDrawingTag, "r:id") ?? null)
        : null;
...
export function ensureLegacyDrawingInSheetXml(sheetXml: string, relationshipId: string): string {
    const legacyDrawingTag = findFirstXmlTag(sheetXml, "legacyDrawing");
    const nextLegacyDrawingXml = buildSelfClosingXmlElement("legacyDrawing", [
        ["r:id", relationshipId],
    ]);
    ...
    const insertionIndex = findWorksheetChildInsertionIndex(
        sheetXml,
        LEGACY_DRAWING_FOLLOWING_TAGS
    );
    return (
        sheetXml.slice(0, insertionIndex) + nextLegacyDrawingXml + sheetXml.slice(insertionIndex)
    );
}
...
export function ensureDefaultContentType(
    ...
    if (
        new RegExp(`<Default\\b[^>]*Extension\\s*=\\s*["']${escapeRegex(extension)}["']`, "i").test(
            contentTypesXml
        )
    ) {
