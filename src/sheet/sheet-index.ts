    ...
    while (cursor < sheetDataInnerEnd) {
        ...
        if (!selfClosing) {
            ...
            while (cellCursor < innerEnd) {
                ...
                const styleId =
                    cellMetadata.styleIdText === undefined
                        ? null
                        : Number(cellMetadata.styleIdText);
                const parsedCell = buildCellSnapshot(
                    workbook,
                    rawType,
                    styleId,
                    sheetXml,
...
function calculateUsedBounds(
    rows: Map<number, LocatedRow>,
    rowNumbers: number[]
): UsedRangeBounds | null {
...
export function buildCellSnapshot(
    ...
    const { formulaAttributesSource, formulaSource, hasSelfClosingValue, valueSource } =
        extractCellContentsFast(xml, innerStart, innerEnd);
    const inlineText =
        rawType === "inlineStr" ? parseStringItemText(xml.slice(innerStart, innerEnd)) : null;
...
export function parseCellTagMetadata(source: string): {
    ...
    while (index < attributesSource.length) {
        while (
            ...
            isXmlWhitespaceCode(attributesSource.charCodeAt(index))
        ) {
            index += 1;
        }
...
function extractCellContentsFast(
    xml: string,
    start: number,
    end: number
): {
...
function resolveSharedFormulaSnapshots(rows: Map<number, LocatedRow>, rowNumbers: number[]): void {
    const sharedAnchors = new Map<
        string,
        { columnNumber: number; formula: string; rowNumber: number }
    >();
    ...
    for (const rowNumber of rowNumbers) {
        ...
        for (const cell of row.cells) {
            ...
            if (
                attributes.has("ref") ||
                (cell.snapshot.formula !== null && cell.snapshot.formula.length > 0)
            ) {
