...
export function parseSheetFreezePane(sheetXml: string): FreezePane | null {
    ...
    const paneTag =
        sheetViewTag && sheetViewTag.innerXml !== null
            ? findFirstXmlTag(sheetViewTag.innerXml, "pane")
            : null;
    ...
    return {
        ...
        topLeftCell:
            getTagAttr(paneTag, "topLeftCell") ?? makeCellAddress(rowCount + 1, columnCount + 1),
...
export function upsertFreezePaneInSheetXml(
    sheetXml: string,
    columnCount: number,
    rowCount: number
): string {
...
export function removeFreezePaneFromSheetXml(sheetXml: string): string {
    ...
    const preferredSelection =
        selections.find(
            (selection) =>
                selection.attributes.find(([name]) => name === "pane")?.[1] === activePane
        ) ??
        selections.find((selection) =>
            selection.attributes.some(([name]) => name === "activeCell" || name === "sqref")
        ) ??
        selections[0];
...
function getSheetViewTags(sheetXml: string): {
    ...
    const sheetViewTag =
        sheetViewsTag && sheetViewsTag.innerXml !== null
            ? findFirstXmlTag(sheetViewsTag.innerXml, "sheetView")
            : null;

    return { sheetViewTag, sheetViewsTag };
}

function ensureXmlAttribute(
...
function buildSelectionXml(attributes: Array<[string, string]>): string {
    return attributes.length === 0
        ? "<selection/>"
        : `<selection ${serializeAttributes(attributes)}/>`;
}
