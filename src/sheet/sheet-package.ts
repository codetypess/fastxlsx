...
import { compareCellAddresses, normalizeRangeRef, parseRangeRef } from "./sheet-address.js";
import {
...
export function findSheetTableReferenceByName(
    ...
    return (
        tableReferences.find((table) => {
            ...
            return (
                getTagAttr(tableTag, "name") === name ||
                getTagAttr(tableTag, "displayName") === name
            );
        }) ?? null
    );
}
...
export function buildTableXml(
    ...
    return (
        ...
        columnNames
            .map(
                (columnName, index) =>
                    `<tableColumn id="${index + 1}" name="${escapeXmlText(columnName)}"/>`
            )
            .join("") +
        `</tableColumns>` +
...
export function appendRelationship(
    ...
    return (
        relationshipsXml.slice(0, insertionIndex) +
        relationshipXml +
        relationshipsXml.slice(insertionIndex)
    );
}
...
export function appendTablePart(sheetXml: string, relationshipId: string): string {
    ...
    if (tablePartsTag && tablePartsTag.innerXml !== null) {
        ...
        const nextTablePartsXml = buildCountedXmlContainer(
            "tableParts",
...
export function removeTablePartsFromSheetXml(sheetXml: string, relationshipIds: string[]): string {
    ...
    const keptTableParts = findXmlTags(tablePartsTag.innerXml, "tablePart")
        ...
        .filter(
            (tablePart) =>
                tablePart.relationshipId && !relationshipIds.includes(tablePart.relationshipId)
        );
