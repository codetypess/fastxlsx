...
export function updateDimensionRef(sheetIndex: SheetIndex): string {
    ...
    return (
        sheetIndex.xml.slice(0, worksheetInnerStart) +
        dimensionXml +
        sheetIndex.xml.slice(worksheetInnerStart)
    );
}

export function formatUsedRangeBounds(bounds: SheetIndex["usedBounds"]): string | null {
    return bounds
        ? formatRangeRef(bounds.minRow, bounds.minColumn, bounds.maxRow, bounds.maxColumn)
        : null;
}
