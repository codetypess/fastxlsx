...
export function parseFormulaTagInfo(cellXml: string): {
...
export function buildRecalculatedFormulaCellXml(
    ...
    const innerWithoutValue = innerXml.replace(
        /<v(?:\s[^>]*)?\/>|<v(?:\s[^>]*)?>[\s\S]*?<\/v>/g,
        ""
    );
    ...
    const nextInnerXml =
        valueXml.length === 0
            ? innerWithoutValue
            : innerWithoutValue.slice(0, formulaTagEnd) +
              valueXml +
              innerWithoutValue.slice(formulaTagEnd);
