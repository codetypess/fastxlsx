...
export function buildStyledRowXml(
    sheetXml: string,
    row: LocatedRow,
    styleId: number | null
): string {
    const serializedAttributes = serializeAttributes(
        buildRowAttributes(row.rowNumber, row.attributesSource, { styleId })
    );
...
export function buildUpdatedRowXml(
    sheetXml: string,
    row: LocatedRow,
    patch: RowDefinitionPatch
): string {
    const serializedAttributes = serializeAttributes(
        buildRowAttributes(row.rowNumber, row.attributesSource, patch)
    );
...
export function transformColumnStyleDefinitions(
    ...
    for (const definition of existingDefinitions) {
        if (mode === "shift") {
            ...
            if (definition.min >= targetColumnNumber) {
                nextDefinitions.push(
                    buildColumnDefinition(
                        definition.min + count,
                        definition.max + count,
            ...
            nextDefinitions.push(
                buildColumnDefinition(definition.min, targetColumnNumber - 1, definition.attributes)
            );
            nextDefinitions.push(
                buildColumnDefinition(
                    targetColumnNumber + count,
                    definition.max + count,
                    definition.attributes
                )
            );
...
function buildRowAttributes(
    ...
    if (
        patch.styleId !== undefined
            ? patch.styleId !== null
            : parseNumericAttribute(attributes, "s") !== null
    ) {
    ...
    const height =
        patch.height !== undefined ? patch.height : parseNumericAttribute(attributes, "ht");
    if (height !== null) {
...
function buildColumnDefinitionWithPatch(
    ...
    const styleId =
        patch.styleId !== undefined
            ? patch.styleId
            : parseNumericAttribute(existingAttributes, "style");
    if (styleId !== null) {
    ...
    const hidden =
        patch.hidden !== undefined
            ? patch.hidden
            : parseBooleanAttribute(existingAttributes, "hidden");
    if (hidden) {
    ...
    const width =
        patch.width !== undefined
            ? patch.width
            : parseNumericAttribute(existingAttributes, "width");
    if (width !== null) {
...
function updateColumnDefinitionInSheetXml(
    ...
    for (const definition of existingDefinitions) {
        ...
        if (definition.min < columnNumber) {
            nextDefinitions.push(
                buildColumnDefinition(definition.min, columnNumber - 1, definition.attributes)
            );
        }

        const nextDefinition = buildColumnDefinitionWithPatch(
            columnNumber,
            columnNumber,
            definition.attributes,
            patch
        ...
        if (columnNumber < definition.max) {
            nextDefinitions.push(
                buildColumnDefinition(columnNumber + 1, definition.max, definition.attributes)
            );
        }
    }
