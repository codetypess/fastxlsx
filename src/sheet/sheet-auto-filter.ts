...
export function setWorksheetAutoFilterDefinitionInSheetXml(
    ...
    nextSheetXml = upsertWorksheetSortStateXml(
        nextSheetXml,
        normalizedDefinition.sortState ?? null
    );
    return nextSheetXml;
}
...
export function setWorksheetAutoFilterRangeInSheetXml(sheetXml: string, range: string): string {
    ...
    if (!currentState) {
        return upsertWorksheetAutoFilterXml(
            sheetXml,
            buildSelfClosingXmlElement("autoFilter", [["ref", normalizedRange]])
        );
    }
    ...
    if (currentState.sortStateLocation === "external") {
        ...
        if (currentExternalSortStateTag) {
            nextSheetXml = replaceXmlTagSource(
                nextSheetXml,
                currentExternalSortStateTag,
                nextSortStateXml ?? ""
            );
        }
    }
...
export function setWorksheetSortStateInSheetXml(
    sheetXml: string,
    sortState: SortStateDefinition | null
): string {
...
export function setTableAutoFilterDefinitionInTableXml(
    ...
    const normalizedDefinition = normalizeAutoFilterDefinition(definition);
    const currentState = parseAutoFilterStateFromTags(
        findFirstXmlTag(tableXml, "autoFilter"),
        null
    );
    const nextRootItems = buildRootItemsForFullDefinition(currentState, normalizedDefinition);
...
export function setTableAutoFilterColumnInTableXml(
    ...
    const nextRootItems = buildRootItemsForColumnUpdate(
        currentState ?? createEmptyState(currentRange),
        column
    );
    const nextAutoFilterXml = buildAutoFilterXml(
        currentRange,
...
export function rewriteSortStateTagWithTransformedRefs(
    ...
    const nextChildXml = childTags
        .map((childTag) => {
            ...
            const nextSortConditionXml = rewriteSortConditionTagWithTransformedRef(
                childTag,
                transformRange
            );
...
function parseAutoFilterDefinitionFromTags(
    ...
    return {
        ...
        columns: state.rootItems
            .filter(
                (item): item is Extract<AutoFilterRootItem, { kind: "column" }> =>
                    item.kind === "column"
            )
...
function parseAutoFilterStateFromTags(
    ...
    let sortStateLocation: "autoFilter" | "external" | null = externalSortStateTag
        ? "external"
        : null;
...
function parseFilterColumnTag(
    ...
    const columnNumber =
        Number.isInteger(colId) && colId >= 0 ? startColumnNumber + colId : startColumnNumber;
...
function parseFiltersColumnTag(filtersTag: XmlTag, columnNumber: number): AutoFilterColumn | null {
    ...
    const unknownTags = childTags.filter(
        (tag) => tag.tagName !== "filter" && tag.tagName !== "dateGroupItem"
    );
...
function parseCustomFilterColumnTag(
    customFiltersTag: XmlTag,
    columnNumber: number
): AutoFilterColumn | null {
...
function parseColorFilterColumnTag(
    colorFilterTag: XmlTag,
    columnNumber: number
): AutoFilterColumn | null {
...
function parseDynamicFilterColumnTag(
    dynamicFilterTag: XmlTag,
    columnNumber: number
): AutoFilterColumn | null {
...
function parseTop10FilterColumnTag(
    top10Tag: XmlTag,
    columnNumber: number
): AutoFilterColumn | null {
...
function parseIconFilterColumnTag(
    iconFilterTag: XmlTag,
    columnNumber: number
): AutoFilterColumn | null {
...
function parseSortStateTag(tag: XmlTag): SortStateDefinition | null {
    ...
    const conditions = findTopLevelXmlTags(tag.innerXml ?? "")
        ...
        .filter(
            (condition): condition is { columnNumber: number; descending?: boolean } =>
                condition !== null
        );
...
function buildRootItemsForFullDefinition(
    ...
    const nextSupportedColumns = new Map(
        definition.columns.map((column) => [
            column.columnNumber,
            buildFilterColumnRootItem(column, definition.range),
        ])
    );
    ...
    for (const column of [...nextSupportedColumns.values()].sort(
...
function rewriteRootItemForRange(
    item: AutoFilterRootItem,
    range: string
): AutoFilterRootItem | null {
    if (item.kind === "column") {
...
function buildFilterColumnRootItem(
    column: AutoFilterColumn,
    range: string
): Extract<AutoFilterRootItem, { kind: "column" }> {
    return {
        kind: "column",
...
function buildFilterColumnXml(column: AutoFilterColumn, range: string): string {
    ...
    const attributes: Array<[string, string]> = [
        ["colId", String(column.columnNumber - startColumn)],
    ];
...
function buildValuesFilterXml(column: Extract<AutoFilterColumn, { kind: "values" }>): string {
    ...
    const childXml = column.values.map((value) =>
        buildSelfClosingXmlElement("filter", [["val", value]])
    );
...
function buildCustomFilterConditionAttributes(
    ...
    switch (condition.operator) {
        ...
        case "contains":
            return [
                ["operator", "equal"],
                ["val", `*${escapeAutoFilterPattern(condition.value)}*`],
            ];
        case "notContains":
            return [
                ["operator", "notEqual"],
...
function buildSortStateXml(sortState: SortStateDefinition): string {
    ...
    const childXml = sortState.conditions.map((condition) =>
        buildSelfClosingXmlElement("sortCondition", [
            [
                "ref",
                formatRangeRef(startRow, condition.columnNumber, endRow, condition.columnNumber),
            ],
...
function upsertWorksheetAutoFilterXml(sheetXml: string, autoFilterXml: string): string {
    ...
    const insertionIndex = findWorksheetChildInsertionIndex(sheetXml, [
        ...WORKSHEET_AUTO_FILTER_FOLLOWING_TAGS,
    ]);
    return sheetXml.slice(0, insertionIndex) + autoFilterXml + sheetXml.slice(insertionIndex);
}

function upsertWorksheetSortStateXml(
    sheetXml: string,
    sortState: SortStateDefinition | null
): string {
...
function upsertTableAutoFilterXml(tableXml: string, autoFilterXml: string): string {
    ...
    const insertionIndex =
        childTags.length > 0
            ? getXmlTagInnerStart(tableTag) + childTags[0].start
            : getXmlTagInnerStart(tableTag);
...
function normalizeAutoFilterDefinition(definition: AutoFilterDefinition): AutoFilterDefinition {
    ...
    const sortState = definition.sortState
        ? normalizeSortStateDefinition(definition.sortState, range)
        : null;
    return {
        range,
        columns,
    ...
    switch (column.kind) {
        ...
        case "custom":
            if (column.conditions.length === 0 || column.conditions.length > 2) {
                throw new XlsxError(
                    `Invalid custom autoFilter condition count for column ${column.columnNumber}`
                );
            }
            return {
                columnNumber: column.columnNumber,
        ...
        case "dateGroup":
            if (column.items.length === 0) {
                throw new XlsxError(
                    `Date-group autoFilter column must include at least one item: ${column.columnNumber}`
                );
            }
            return {
                columnNumber: column.columnNumber,
        ...
        case "color":
            if (!Number.isInteger(column.dxfId) || column.dxfId < 0) {
                throw new XlsxError(
                    `Invalid color autoFilter dxfId for column ${column.columnNumber}`
                );
            }
            return {
                columnNumber: column.columnNumber,
        ...
        case "icon":
            if (
                column.iconId !== undefined &&
                (!Number.isInteger(column.iconId) || column.iconId < 0)
            ) {
                throw new XlsxError(
                    `Invalid icon autoFilter iconId for column ${column.columnNumber}`
                );
            }
            return {
                columnNumber: column.columnNumber,
...
function normalizeSortStateDefinition(
    sortState: SortStateDefinition,
    filterRange: string
): SortStateDefinition {
...
function assertColumnInRange(columnNumber: number, range: string): void {
    if (!isColumnInRange(columnNumber, range)) {
        throw new XlsxError(
            `AutoFilter column ${columnNumber} is outside range ${normalizeRangeRef(range)}`
        );
    }
}
...
function rewriteFilterColumnSourceForRange(
    source: string,
    columnNumber: number,
    range: string
): string {
...
function deleteShiftColumnNumber(
    columnNumber: number,
    targetColumnNumber: number,
    count: number
): number | null {
