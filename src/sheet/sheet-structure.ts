...
export function transformRowXml(
    ...
    const nextRowAttributes = rowAttributes.map(([name, value]) => {
        if (name === "r") {
            return [name, String(shiftRowNumber(Number(value), targetRowNumber, rowCount))] as [
                string,
                string,
            ];
        }
        ...
        if (name === "spans") {
            return [name, shiftRowSpans(value, targetColumnNumber, columnCount)] as [
                string,
                string,
            ];
        }
...
export function deleteRowTransform(
    ...
    const nextRowAttributes: Array<[string, string]> = [
        ["r", String(nextRowNumber)],
        ...rowAttributes,
    ];
...
export function shiftRangeRefColumns(
    range: string,
    targetColumnNumber: number,
    count: number
): string {
...
export function deleteRangeRefColumns(
    range: string,
    targetColumnNumber: number,
    count: number
): string | null {
...
export function deleteRangeRefRows(
    range: string,
    targetRowNumber: number,
    count: number
): string | null {
...
export function transformWorksheetStructureReferences(
    ...
    for (const tagName of WORKSHEET_REF_TAGS) {
        nextSheetXml = rewriteWorksheetReferenceTag(
            nextSheetXml,
            tagName,
            "ref",
            false,
    ...
    nextSheetXml = rewriteCountedContainer(
        nextSheetXml,
        "dataValidations",
        "dataValidation",
        "count"
...
export function translateFormulaReferences(
    ...
    while (cursor < formula.length) {
        ...
        if (rangeMatch && matchesTranslatedFormulaReference(previous)) {
            ...
            nextFormula +=
                leftRef === null || rightRef === null ? "#REF!" : `${leftRef}:${rightRef}`;
            cursor += fullMatch.length;
            continue;
        }
...
export function deleteFormulaReferences(
    ...
    while (cursor < formula.length) {
        ...
        if (
            isColumnDeleted(columnNumber, targetColumnNumber, columnCount) ||
            isRowDeleted(rowNumber, targetRowNumber, rowCount)
        ) {
            nextFormula += "#REF!";
        } else {
...
export function renameSheetFormulaReferences(
    ...
    while (cursor < formula.length) {
        ...
        if (rangeMatch) {
            ...
            const nextStartSheetRef = renameSheetReferencePrefix(
                startSheetRef,
                previousSheetName,
                nextSheetName
            );
            const nextEndSheetRef = renameSheetReferencePrefix(
                endSheetRef,
                previousSheetName,
                nextSheetName
            );
...
function transformCellXml(
    ...
    const nextAttributes = attributes.map(([name, value]) => {
        if (name === "r") {
            return [
                name,
                shiftCellAddress(value, targetColumnNumber, columnCount, targetRowNumber, rowCount),
            ] as [string, string];
        }
...
function deleteRowCellTransform(
    ...
    const attributes = parseAttributes(cell.attributesSource).map(([name, value]) => {
        if (name === "r") {
            ...
            return [
                name,
                makeCellAddress(
                    deleteShiftRowNumber(rowNumber, targetRowNumber, count),
                    columnNumber
                ),
            ] as [string, string];
        }
    ...
    return deleteTransformCellInnerXml(
        cellXml,
        attributes,
        sheetName,
        0,
    ...
    const attributes = parseAttributes(cell.attributesSource).map(([name, value]) => {
        if (name === "r") {
            ...
            return [
                ...
                makeCellAddress(
                    rowNumber,
                    deleteShiftColumnNumber(columnNumber, targetColumnNumber, count)
                ),
            ] as [string, string];
        }
        ...
        const nextFormulaAttributes = formulaAttributes.map(([name, value]) => {
            if (name === "ref") {
                ...
                return nextRange === null
                    ? ([name, "#REF!"] as [string, string])
                    : ([name, nextRange] as [string, string]);
            }
...
function isColumnDeleted(columnNumber: number, targetColumnNumber: number, count: number): boolean {
    return (
        ...
        columnNumber <= targetColumnNumber + count - 1
    );
}
...
function isRowDeleted(rowNumber: number, targetRowNumber: number, count: number): boolean {
    return (
        ...
        rowNumber <= targetRowNumber + count - 1
    );
}
...
function rewriteWorksheetReferenceTag(
    ...
    return sheetXml.replace(regex, (match, attributesSource, bodySource) => {
        ...
        const nextValue = transformWorksheetReferenceValue(
            currentValue,
            multipleRanges,
            transformRange
        );
        ...
        const tagOpen =
            serializedAttributes.length > 0 ? `<${tagName} ${serializedAttributes}` : `<${tagName}`;
...
function rewriteCountedContainer(
    ...
    return sheetXml.replace(regex, (_match, attributesSource, innerXml) => {
        const childMatches =
            innerXml.match(new RegExp(`<${escapeRegex(childTagName)}\\b`, "g")) ?? [];
...
function translateFormulaReference(
    ...
    const nextRowNumber = rowDollar === "$" ? Number(rowText) : Number(rowText) + rowDelta;
