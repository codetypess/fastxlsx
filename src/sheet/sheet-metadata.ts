    ...
    return parseDataValidationEntries(dataValidationsTag.innerXml)
        .map((validationTag) => {
            ...
            return {
                ...
                showInputMessage: parseOptionalXmlBoolean(
                    getTagAttr(validationTag, "showInputMessage")
                ),
                showErrorMessage: parseOptionalXmlBoolean(
                    getTagAttr(validationTag, "showErrorMessage")
                ),
...
export function upsertDataValidationInSheetXml(
    sheetXml: string,
    dataValidationXml: string,
    range: string
): string {
    const normalizedRange = normalizeSqref(range);
    const dataValidationsTag = findFirstXmlTag(sheetXml, "dataValidations");
    const dataValidations = (
        (dataValidationsTag?.innerXml ?? "")
            ? parseDataValidationEntries(dataValidationsTag?.innerXml ?? "").map(
                  (validationTag) => ({
    ...
    const insertionIndex = findWorksheetChildInsertionIndex(
        sheetXml,
        DATA_VALIDATIONS_FOLLOWING_TAGS
    );
    return (
        sheetXml.slice(0, insertionIndex) + nextDataValidationsXml + sheetXml.slice(insertionIndex)
    );
}
...
export function removeDataValidationFromSheetXml(sheetXml: string, range: string): string {
    ...
    const keptDataValidations = parseDataValidationEntries(dataValidationsTag.innerXml).filter(
        (validationTag) =>
            normalizeSqref(getTagAttr(validationTag, "sqref") ?? "") !== normalizedRange
    );
...
export function parseSheetProtection(sheetXml: string): SheetProtection | null {
    ...
    return {
        ...
        selectUnlockedCells: parseOptionalXmlBoolean(
            getTagAttr(protectionTag, "selectUnlockedCells")
        ),
...
export function upsertSheetProtectionInSheetXml(
    ...
    const insertionIndex = findWorksheetChildInsertionIndex(
        sheetXml,
        SHEET_PROTECTION_FOLLOWING_TAGS
    );
...
export function parseSheetHyperlinks(
    ...
    return findXmlTags(sheetXml, "hyperlink")
        .map((tag) => {
...
export function buildInternalHyperlinkXml(
    address: string,
    location: string,
    tooltip?: string
): string {
...
export function buildExternalHyperlinkXml(
    address: string,
...
export function upsertHyperlinkInSheetXml(
    sheetXml: string,
    hyperlinkXml: string,
    address: string
): string {
...
function appendOptionalBooleanAttribute(
    attributes: Array<[string, string]>,
    name: string,
