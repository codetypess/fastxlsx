...
export function resolveCloneStylePatch(
    ...
    return typeof addressOrRowNumber === "number"
        ? (patch ?? {})
...
export function resolveSetFontPatch(
    ...
    return typeof addressOrRowNumber === "number"
        ? (patch ?? {})
        : (columnOrPatch as CellFontPatch);
}
...
export function resolveSetFillPatch(
    ...
    return typeof addressOrRowNumber === "number"
        ? (patch ?? {})
        : (columnOrPatch as CellFillPatch);
}
...
export function resolveSetBorderPatch(
    ...
    return typeof addressOrRowNumber === "number"
        ? (patch ?? {})
        : (columnOrPatch as CellBorderPatch);
}
...
export function resolveSetStyleId(
    ...
    const nextStyleId =
        typeof addressOrRowNumber === "number"
            ? (styleId ?? null)
            : (columnOrStyleId as number | null);
