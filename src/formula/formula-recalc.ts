...
import type { CellError, CellSnapshot, CellValue, RecalculateSummary } from "../types.js";
...
const FORMULA_ERROR_TEXTS = Object.keys(FORMULA_ERROR_CODES).sort(
...
class FormulaParser {
    ...
    private parsePrimary(): AstNode {
        ...
        if (
            (token.type === "identifier" || token.type === "quoted_identifier") &&
            this.peek(1).type === "bang"
        ) {
    ...
    private expect(type: Token["type"]): Token {
        ...
        if (token.type !== type) {
            throw new XlsxError(
                `Expected ${type} in formula, received ${token.value || token.type}`
            );
        }
...
class FormulaRuntime {
    ...
    evaluateFormulaCell(sheetName: string, address: string): ScalarValue {
        ...
        if (!definition) {
            return scalarFromSnapshot(
                this.requireSheet(resolvedSheetName).readCellSnapshot(address)
            );
        }
        ...
        this.enterStack(
            key,
            `Circular formula reference: ${definition.sheetName}!${definition.address}`
        );
        try {
    ...
    private evaluateFunction(
        ...
        switch (name) {
            case "IF": {
                ...
                const condition = ensureScalarValue(
                    this.evaluateNode(node.arguments[0]!, currentSheetName)
                );
        ...
        const evaluatedArguments = node.arguments.map((argument) =>
            this.evaluateNode(argument, currentSheetName)
        );
        ...
        switch (name) {
            case "AND":
                return scalarResult({
                    error: null,
                    value: evaluatedArguments.every((argument) =>
                        toBoolean(ensureScalarValue(argument))
                    ),
                });
            case "AVERAGE": {
                const numbers = collectNumericValues(evaluatedArguments, {
                    ignoreText: true,
                    includeBlankAsZero: false,
                });
            ...
            case "OR":
                return scalarResult({
                    error: null,
                    value: evaluatedArguments.some((argument) =>
                        toBoolean(ensureScalarValue(argument))
                    ),
                });
            case "SUM": {
    ...
    private evaluateMatchFunction(
        ...
        const lookupValue = ensureScalarValue(
            this.evaluateNode(node.arguments[0]!, currentSheetName)
        );
        ...
        const lookupArray = coerceToRangeValue(
            this.evaluateNode(node.arguments[1]!, currentSheetName)
        );
        ...
        return scalarResult(
            index === -1 ? makeErrorValue("#N/A") : { error: null, value: index + 1 }
        );
    }
    ...
    private evaluateVlookupFunction(
        ...
        const rangeLookup = resolveRangeLookup(
            node.arguments[3]
                ? ensureScalarValue(this.evaluateNode(node.arguments[3]!, currentSheetName))
                : null
        );
        ...
        return scalarResult(
            rowIndex === -1
                ? makeErrorValue("#N/A")
                : getRangeValue(table, rowIndex, columnIndex - 1)
        );
    }

    private evaluateName(
        node: Extract<AstNode, { kind: "name" }>,
        currentSheetName: string
    ): EvaluatedValue {
        ...
        const definition =
            this.workbook.getDefinedName(node.name, currentSheetName) ??
            this.workbook.getDefinedName(node.name);
        if (definition === null) {
            return scalarResult(makeErrorValue("#NAME?"));
        }
    ...
    private evaluateNode(node: AstNode, currentSheetName: string): EvaluatedValue {
        switch (node.kind) {
            ...
            case "reference":
                return scalarResult(
                    this.readCellValue(node.sheetName ?? currentSheetName, node.address)
                );
            case "string":
                return scalarResult({ error: null, value: node.value });
            case "unary":
                return scalarResult(
                    evaluateUnary(
                        node.operator,
                        ensureScalarValue(this.evaluateNode(node.operand, currentSheetName))
                    )
                );
            case "binary":
                return scalarResult(
    ...
    private getFormulaCellDefinition(
        sheetName: string,
        address: string
    ): FormulaCellDefinition | undefined {
...
export function recalculateCellFormula(
    workbook: Workbook,
    sheet: Sheet,
    address: string
): CellSnapshot {
...
function applyFormulaResults(
    ...
    return {
        ...
        sheets:
            targetSheets?.size ??
            new Set([...results.keys()].map((key) => splitFormulaKey(key).sheetName)).size,
        updated,
    };
}
...
function collectRangeValues(
    ...
    const range = parseRangeRef(
        `${makeCellAddress(start.rowNumber, start.columnNumber)}:${makeCellAddress(end.rowNumber, end.columnNumber)}`
    );
    ...
    for (let rowNumber = range.startRow; rowNumber <= range.endRow; rowNumber += 1) {
        for (
            ...
            values.push(
                runtime.evaluateFormulaCell(sheetName, makeCellAddress(rowNumber, columnNumber))
            );
        }
    }
...
function evaluateBinary(operator: string, left: ScalarValue, right: ScalarValue): ScalarValue {
    ...
    switch (operator) {
        case "+":
            return {
                error: null,
                value: (leftNumber.value as number) + (rightNumber.value as number),
            };
        case "-":
            return {
                error: null,
...
function findApproximateLookupIndex(
    values: ScalarValue[],
    lookupValue: ScalarValue,
    matchType: number
): number {
    let bestIndex = -1;
    ...
    for (const [index, candidate] of values.entries()) {
        ...
        if (
            (matchType > 0 && candidateComparison > 0) ||
            (matchType < 0 && candidateComparison < 0)
        ) {
            continue;
        }
...
function resolveColumnIndex(value: ScalarValue, width: number): ScalarValue | number {
    ...
    if (
        numericValue.error !== null ||
        typeof numericValue.value !== "number" ||
...
function toNumber(value: ScalarValue, allowText = false): ScalarValue {
    ...
    return allowText
        ? { error: makeErrorValue("#VALUE!").error, value: null }
...
function tokenizeFormula(source: string): Token[] {
    ...
    while (index < source.length) {
        ...
        const errorText = FORMULA_ERROR_TEXTS.find(
            (candidate) => source.slice(index, index + candidate.length).toUpperCase() === candidate
        );
        if (errorText) {
