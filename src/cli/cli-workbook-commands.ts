...
export function registerWorkbookCommands(program: Command, io: CliCommandIo): void {
    ...
    program
        ...
        .option(
            "--header-row <row>",
            "header row used for the sheet preview",
            parsePositiveInteger,
            1
        )
    ...
    definedNameCommand
        ...
        .action(async (file: string, options: { scope?: string }) => {
            ...
            const normalizedScope = options.scope
                ? (workbook.tryGetSheet(options.scope)?.name ?? options.scope)
                : undefined;
            const definedNames = options.scope
                ? workbook
                      .getDefinedNames()
                      .filter((definedName) => definedName.scope === (normalizedScope ?? null))
        ...
        .action(async (file: string, options: { name: string; scope?: string }) => {
            ...
            const definedName =
                workbook
                    ...
                    .find(
                        (candidate) =>
                            candidate.name === options.name && candidate.scope === normalizedScope
                    ) ?? null;
            writeJson(io.stdout, {
                definedName,
                file: inputPath,
    ...
    program
        ...
        .action(
            async (
                ...
                const actionCount =
                    Number(Boolean(options.clear)) + Number(options.color !== undefined);
                if (actionCount !== 1) {
                ...
                workbook
                    ...
                    .setBackgroundColor(options.cell, options.clear ? null : options.color!);
                await workbook.save(outputPath);
                writeJson(io.stdout, {
    ...
    program
        ...
        .action(
            async (
                ...
                if (actionCount !== 1) {
                    throw new Error(
                        "Exactly one of --value, --text, --formula, or --clear is required"
                    );
                }
        ...
        if (shouldUseStructuredTable(sheet, profile)) {
            return {
                commands: {
                    ...
                    inspect: buildTableInspectCommand(
                        filePath,
                        sheetName,
                        profile.headerRow,
                        profile.dataStartRow
                    ),
                    list: buildTableListCommand(
                        filePath,
                        sheetName,
                        profile.headerRow,
                        profile.dataStartRow
...
function shouldUseStructuredTable(
    ...
    const firstRowHeaders = trimTrailingEmptyStrings(sheet.getHeaders(1)).map((value) =>
        value.trim()
    );
...
function buildConfigTableListCommand(
    filePath: string,
    sheetName: string,
    headerRow: number
): string {
