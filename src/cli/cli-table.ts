        ...
        return {
            headers:
                record.headers === undefined
                    ? undefined
                    : assertStringArray(record.headers, `${filePath}.headers`),
...
export async function readTableProfiles(filePath: string): Promise<Record<string, TableProfile>> {
    ...
    for (const [name, rawProfile] of Object.entries(profiles)) {
        ...
        next[name] = {
            dataStartRow: assertPositiveInteger(
                profile.dataStartRow,
                `${filePath}.profiles.${name}.dataStartRow`
            ),
            headerRow: assertPositiveInteger(
                profile.headerRow,
                `${filePath}.profiles.${name}.headerRow`
            ),
            keyFields:
                profile.keyFields === undefined
                    ...
                    : assertStringArray(
                          profile.keyFields,
                          `${filePath}.profiles.${name}.keyFields`
                      ),
...
export function findStructuredTableRow(
    ...
    return (
        listStructuredTableRows(sheet, headerRow, dataStartRow).find((row) =>
            matchesKey(row.record, keyFields, key)
        ) ?? null
    );
}
...
export function writeStructuredTableRecords(
    ...
    const existingRows = listStructuredTableRows(sheet, headerRow, dataStartRow).map(
        (row) => row.row
    );
...
export function listConfigTableRows(sheet: CliSheet, headerRow: number): ConfigTableRow[] {
...
function inferTableDataStartRow(sheet: CliSheet, headerRow: number): number {
    for (let row = headerRow + 1; row <= sheet.rowCount; row += 1) {
        ...
        if (
            ...
            (isMetadataMarker(firstValue) || looksLikeTypeDescriptor(firstValue))
        ) {
            continue;
        }
...
function scoreHeaderRowCandidate(sheet: CliSheet, row: number): number {
    const values = sheet.getRow(row);
    const headers = trimTrailingEmptyStrings(
    ...
    if (
        nonEmptyHeaders.some(
            (header) => header === "id" || header === "key" || /^key\d+$/.test(header)
        )
    ) {
        score += 4;
    }
