...
export function registerApplyCommands(program: Command, io: CliCommandIo): void {
    program
        .command("apply")
        ...
        .action(
            async (
                ...
                const configuredOutput = document.output
                    ? resolveFrom(io.cwd, document.output)
                    : undefined;
...
async function readOpsDocument(filePath: string): Promise<OpsDocument> {
    ...
    if (Array.isArray(parsed)) {
        return {
            actions: parsed.map((item, index) =>
                parseWorkbookOperation(item, `${filePath}[${index}]`)
            ),
        };
    }
    ...
    return {
        actions: actions.map((item, index) =>
            parseWorkbookOperation(item, `${filePath}.actions[${index}]`)
        ),
        output:
            record.output === undefined
                ? undefined
                : assertString(record.output, `${filePath}.output`),
    };
}
    ...
    switch (action.type) {
        ...
        case "setHeaders":
            workbook
                ...
                .setHeaders(action.headers, action.headerRow ?? 1, action.startColumn ?? 1);
            return;
        case "setRecord":
            workbook
                .getSheet(action.sheet)
                .setRecord(action.row, action.record, action.headerRow ?? 1);
            return;
        case "setRecords":
