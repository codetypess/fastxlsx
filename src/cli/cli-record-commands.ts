...
export function registerRecordCommands(program: Command, io: CliCommandIo): void {
    ...
    sheetCommand
        ...
        .option(
            "--mode <mode>",
            "replace rewrites records; append adds rows; update patches matched rows; upsert inserts missing rows and replaces matched rows"
        )
            ...
            async (
                ...
                const result =
                    format === "json"
                        ...
                        : sheet.importRecords(
                              parseCsvAsRecords(await readFile(sourcePath, "utf8"), {
                                  inferTypes: options.inferTypes !== false,
                                  trimHeaders: options.trimHeaders !== false,
                                  trimValues: options.trimValues === true,
                              }),
    ...
    layoutCommand
        ...
        .option(
            "--columns <json>",
            "JSON array of column labels to inspect; defaults to all explicit column widths"
        )
        .option(
            "--rows <json>",
            "JSON array of row numbers to inspect; defaults to all explicit row heights"
        )
        .action(
            async (
                file: string,
                ...
                const columns =
                    options.columns === undefined
                        ? undefined
                        : parseJsonStringArray(options.columns, "--columns");
                const rows =
                    options.rows === undefined
                        ? undefined
                        : parseJsonNumberArray(options.rows, "--rows");
                ...
                writeJson(io.stdout, {
                    ...
                    columnWidths:
                        columns === undefined
                            ...
                            : Object.fromEntries(
                                  columns.map((column) => [column, sheet.getColumnWidth(column)])
                              ),
            ...
            async (
                ...
                if (options.columnWidths) {
                    for (const [column, width] of Object.entries(
                        parseWorksheetNumberMap(options.columnWidths, "--column-widths")
                    )) {
                        sheet.setColumnWidth(column, width);
                    }
                }
                if (options.rowHeights) {
                    for (const [rowNumberText, height] of Object.entries(
                        parseWorksheetNumberMap(options.rowHeights, "--row-heights")
                    )) {
                ...
                if (options.clearFreeze) {
                    ...
                    options.freezeColumns !== undefined ||
                    options.freezeRows !== undefined
                ) {
                ...
                if (options.clearPrintTitles) {
                    ...
                    options.printTitleColumns !== undefined ||
                    options.printTitleRows !== undefined
                ) {
    ...
    commentCommand
        ...
        .action(
            async (
                ...
                const workbook = await Workbook.open(inputPath);
                const comment = workbook
                    .getSheet(options.sheet)
                    .setComment(options.cell, options.text, {
        ...
        .action(
            async (
                ...
                sheet.setAutoFilterDefinition(
                    parseJsonAutoFilterDefinition(options.definition, "--definition")
                );
                await workbook.save(outputPath);
                writeJson(io.stdout, {
        ...
        .action(
            async (
                ...
                sheet.clearAutoFilterColumns(
                    options.columnNumbers
                        ? parseJsonNumberArray(options.columnNumbers, "--column-numbers")
                        : undefined
                );
                await workbook.save(outputPath);
                writeJson(io.stdout, {
    ...
    validationCommand
        ...
        .option(
            "--show-input-message <value>",
            "show the input prompt (true/false)",
            parseBooleanValue
        )
        .option(
            "--show-error-message <value>",
            "show the error message (true/false)",
            parseBooleanValue
        )
        .option(
            "--show-drop-down <value>",
            "show the dropdown arrow (true/false)",
            parseBooleanValue
        )
    ...
    program
        ...
        .action(
            async (
                file: string,
                options: { headerRow: number; output?: string; sheet: string }
            ) => {
                const result = await getRecords(
                    resolveFrom(io.cwd, file),
                    options.sheet,
...
function parseSheetImportMode(
    value?: string
): "append" | "replace" | "update" | "upsert" | undefined {
    if (value === undefined) {
        return undefined;
    }
    ...
    switch (kind) {
        case "values":
            return {
                columnNumber,
                kind,
                values: parseJsonStringArray(
                    JSON.stringify(record.values ?? []),
                    `${label}.values`
                ),
                ...(record.includeBlank === undefined
                    ? {}
                    : {
                          includeBlank: assertBooleanValue(
                              record.includeBlank,
                              `${label}.includeBlank`
                          ),
                      }),
        ...
        case "custom":
            return {
                ...
                conditions: assertAutoFilterConditionArray(
                    record.conditions,
                    `${label}.conditions`
                ),
            };
        ...
        case "dynamic":
            return {
                ...
                ...(record.val === undefined
                    ? {}
                    : { val: assertFiniteNumber(record.val, `${label}.val`) }),
                ...(record.maxVal === undefined
                    ? {}
                    : { maxVal: assertFiniteNumber(record.maxVal, `${label}.maxVal`) }),
                ...(record.valIso === undefined
                    ? {}
                    : { valIso: assertString(record.valIso, `${label}.valIso`) }),
                ...(record.maxValIso === undefined
                    ? {}
                    : { maxValIso: assertString(record.maxValIso, `${label}.maxValIso`) }),
            };
        case "top10":
            return {
                columnNumber,
                ...
                ...(record.filterValue === undefined
                    ? {}
                    : {
                          filterValue: assertFiniteNumber(
                              record.filterValue,
                              `${label}.filterValue`
                          ),
                      }),
        ...
        case "icon":
            return {
                columnNumber,
                kind,
                ...(record.iconSet === undefined
                    ? {}
                    : { iconSet: assertString(record.iconSet, `${label}.iconSet`) }),
...
function assertAutoFilterConditionArray(value: unknown, label: string): AutoFilterCondition[] {
    ...
    return values.map((item, index) => {
        ...
        const operator = assertAutoFilterConditionOperator(
            record.operator,
...
function assertDateGroupItemArray(
    value: unknown,
    label: string
): Array<{
    year: number;
    ...
    return values.map((item, index) => {
        ...
        return {
            ...
            ...(record.month === undefined
                ? {}
                : { month: assertPositiveIntegerValue(record.month, `${label}[${index}].month`) }),
            ...(record.day === undefined
                ? {}
                : { day: assertPositiveIntegerValue(record.day, `${label}[${index}].day`) }),
            ...(record.hour === undefined
                ? {}
                : { hour: assertPositiveIntegerValue(record.hour, `${label}[${index}].hour`) }),
            ...(record.minute === undefined
                ? {}
                : {
                      minute: assertPositiveIntegerValue(
                          record.minute,
                          `${label}[${index}].minute`
                      ),
                  }),
            ...(record.second === undefined
                ? {}
                : {
                      second: assertPositiveIntegerValue(
                          record.second,
                          `${label}[${index}].second`
                      ),
                  }),
            dateTimeGrouping: assertDateTimeGrouping(
                record.dateTimeGrouping,
                `${label}[${index}].dateTimeGrouping`
            ),
        };
    });
}
...
function assertOptionalSortStateDefinition(
    ...
    const conditions = assertArray(record.conditions, `${label}.conditions`).map((item, index) => {
        ...
        return {
            columnNumber: assertPositiveIntegerValue(
                condition.columnNumber,
                `${label}.conditions[${index}].columnNumber`
            ),
            ...(condition.descending === undefined
                ? {}
                : {
                      descending: assertBooleanValue(
                          condition.descending,
                          `${label}.conditions[${index}].descending`
                      ),
                  }),
        };
    });
...
function assertAutoFilterConditionOperator(
    value: unknown,
    label: string
): AutoFilterCondition["operator"] {
    if (
        value === "equals" ||
        value === "notEquals" ||
        value === "greaterThan" ||
