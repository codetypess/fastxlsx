...
export function parseStylesXml(path: string, xml: string): StylesCache {
    ...
    return {
        ...
        cellXfs: findXmlTags(cellXfsTag.innerXml, "xf").map((tag) =>
            parseCellStyle(tag.attributesSource, tag.innerXml ?? "")
        ),
    };
}
...
function parseBorder(borderXml: string): ParsedBorder {
    ...
    const [horizontalTag, remainingAfterHorizontal] = takeFirstXmlTagByName(
        remainingXml,
        "horizontal"
    );
...
function parseFont(fontXml: string): ParsedFont {
    ...
    const [vertAlignTag, remainingAfterVertAlign] = takeFirstXmlTagByName(
        remainingXml,
        "vertAlign"
    );
...
function parseCellStyle(attributesSource: string, innerXml: string): ParsedCellStyle {
    ...
    const alignmentAttributes = alignmentTag
        ? parseAttributes(alignmentTag.attributesSource)
        : null;
...
function parseAlignmentDefinition(attributes: Array<[string, string]>): CellStyleAlignment {
    ...
    assignNumberAttribute(
        alignment,
        "textRotation",
        findAttributeValue(attributes, "textRotation")
    );
    ...
    assignNumberAttribute(
        alignment,
        "relativeIndent",
        findAttributeValue(attributes, "relativeIndent")
    );
    assignBooleanAttribute(
        alignment,
        "justifyLastLine",
        findAttributeValue(attributes, "justifyLastLine")
    );
    assignNumberAttribute(
        alignment,
        "readingOrder",
        findAttributeValue(attributes, "readingOrder")
    );
...
function assignStringAttribute(
    target: CellStyleAlignment,
    name: keyof CellStyleAlignment,
    value?: string
): void {
    if (value !== undefined) {
