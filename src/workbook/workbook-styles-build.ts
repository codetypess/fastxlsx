...
export function buildPatchedCellXfXml(sourceStyle: ParsedCellStyle, patch: CellStylePatch): string {
    ...
    const alignmentAttributes = applyAlignmentPatch(
        sourceStyle.alignmentAttributes,
        patch.alignment
    );
    const alignmentXml = alignmentAttributes
        ? buildSelfClosingTag("alignment", alignmentAttributes)
        : "";
...
export function cloneCellStyleDefinition(
    style: CellStyleDefinition | null
...
export function cloneCellFontDefinition(
    font: CellFontDefinition | null
...
export function cloneCellFillDefinition(
    fill: CellFillDefinition | null
...
export function cloneCellBorderDefinition(
    border: CellBorderDefinition | null
...
function applyCellStylePatch(
    ...
    nextAttributes = applyOptionalBooleanPatch(
        nextAttributes,
        "applyNumberFormat",
        patch.applyNumberFormat
    );
    ...
    nextAttributes = applyOptionalBooleanPatch(
        nextAttributes,
        "applyAlignment",
        patch.applyAlignment
    );
    nextAttributes = applyOptionalBooleanPatch(
        nextAttributes,
        "applyProtection",
        patch.applyProtection
    );
...
function applyAlignmentPatch(
    ...
    nextAttributes = applyOptionalIntegerPatch(
        nextAttributes,
        "relativeIndent",
        patch.relativeIndent
    );
    nextAttributes = applyOptionalBooleanPatch(
        nextAttributes,
        "justifyLastLine",
        patch.justifyLastLine
    );
    ...
    if (font.underline !== null) {
        parts.push(
            font.underline === "single"
                ? "<u/>"
                : buildSelfClosingTag("u", [["val", font.underline]])
        );
    }
    if (font.strike) {
        parts.push("<strike/>");
...
function buildFillChildXml(fill: CellFillDefinition): string {
    ...
    const attributes =
        fill.patternType === null
            ? []
            : ([["patternType", fill.patternType]] as Array<[string, string]>);
    const colorXml =
        (fill.fgColor
            ? buildSelfClosingTag("fgColor", buildFillColorAttributes(fill.fgColor))
            : "") +
        (fill.bgColor
            ? buildSelfClosingTag("bgColor", buildFillColorAttributes(fill.bgColor))
            : "");
...
function buildBorderSideXml(tagName: string, side: CellBorderSideDefinition | null): string {
    ...
    const colorXml = side.color
        ? buildSelfClosingTag("color", buildBorderColorAttributes(side.color))
        : "";
    if (colorXml.length === 0) {
...
function applyBorderPatch(
    sourceBorder: CellBorderDefinition,
    ...
    return {
        ...
        diagonalDown:
            patch.diagonalDown === undefined ? sourceBorder.diagonalDown : patch.diagonalDown,
...
function cloneCellBorderSideDefinition(
    side: CellBorderSideDefinition | null
): CellBorderSideDefinition | null {
    if (!side) {
        return null;
    }
...
function applyOptionalIntegerPatch(
    ...
    return value === undefined
        ? attributes
        : upsertAttribute(attributes, name, value === null ? null : String(value));
}
...
function applyOptionalBooleanPatch(
    ...
    return value === undefined
        ? attributes
        : upsertAttribute(attributes, name, value === null ? null : value ? "1" : "0");
}
