...
export function toRelationshipTarget(workbookDir: string, path: string): string {
    return workbookDir && path.startsWith(`${workbookDir}/`)
        ? path.slice(workbookDir.length + 1)
        : path;
}
...
export function renameSheetInWorkbookXml(
    ...
    return rewriteDefinedNamesInWorkbookXml(renamedWorkbookXml, (tag) => {
        ...
        const nextNameText = renameSheetFormulaReferences(
            nameText,
            currentSheetName,
            nextSheetName
        );
...
export function reorderWorkbookXmlSheets(
    ...
    const nextWorkbookXml = replaceXmlTagSource(
        workbookXml,
        sheetsTag,
        `<sheets>${reorderedSheetsXml}</sheets>`
    );
    const nextActiveTab = localSheetIdMap.get(
        parseActiveSheetIndex(workbookXml, currentSheets.length)
    );
    const rewrittenDefinedNamesWorkbookXml = rewriteDefinedNamesInWorkbookXml(
        nextWorkbookXml,
        (tag) => {
...
export function updateActiveSheetInWorkbookXml(
    workbookXml: string,
...
export function removeSheetFromWorkbookXml(
    ...
    return rewriteDefinedNamesInWorkbookXml(withoutSheet, (tag) => {
        const attributes = parseAttributes(tag.attributesSource);
        const localSheetIdIndex = attributes.findIndex(([name]) => name === "localSheetId");
        const localSheetIdText =
            localSheetIdIndex === -1 ? undefined : attributes[localSheetIdIndex]?.[1];
...
function buildLocalSheetIdMap(
    currentSheets: Array<{ relationshipId: string }>,
    nextSheets: Array<{ relationshipId: string }>
): Map<number, number> {
...
function replaceNestedXmlTagSource(
    xml: string,
    parentTag: XmlTag,
    childTag: XmlTag,
    nextSource: string
...
function rewriteSheetsInWorkbookXml(
    ...
    return {
        ...
        workbookXml: replaceXmlTagSource(
            workbookXml,
            sheetsTag,
            `<sheets>${nextSheetSources.join("")}</sheets>`
        ),
    };
}
...
function updateWorkbookViewActiveTab(workbookXml: string, activeSheetIndex: number): string {
    ...
    if (!bookViewsTag) {
        ...
        return (
            workbookXml.slice(0, sheetsTag.start) +
            `<bookViews>${workbookViewXml}</bookViews>` +
            workbookXml.slice(sheetsTag.start)
        );
    }

    const workbookViewTag =
        bookViewsTag.innerXml === null
            ? null
            : findFirstXmlTag(bookViewsTag.innerXml, "workbookView");
    if (!workbookViewTag) {
