...
export function shouldResetWorkbookContext(
    context: WorkbookContext | undefined,
    path: string
): boolean {
    return !!(context && (context.workbookPath === path || context.workbookRelsPath === path));
}

export function shouldResetStylesCache(
    context: WorkbookContext | undefined,
    path: string
): boolean {
    return !!(
        context &&
        (context.stylesPath === path ||
            context.workbookPath === path ||
            context.workbookRelsPath === path)
    );
}

export function shouldResetSharedStringsCache(
    context: WorkbookContext | undefined,
    path: string
): boolean {
