...
export function scanValueWindowCellFast(
  ...
  while (cellCursor < rowInnerEnd) {
    ...
    const cellMetadata = parseValueWindowCellTagMetadataFast(xml, cellStart + 2, cellOpenTagEnd);
    const cellEnd = cellMetadata.selfClosing ? cellOpenTagEnd + 1 : xml.indexOf("</c>", cellOpenTagEnd + 1);
    ...
    return {
      ...
      innerEnd,
      innerStart,
      logical: hasLogicalValueWindowCellContent(xml, innerStart, innerEnd, cellMetadata.rawType),
