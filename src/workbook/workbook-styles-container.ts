...
export function replaceFillInStylesXml(stylesXml: string, fillId: number, fillXml: string): string {
    return replaceIndexedXmlInStylesContainer(
        ...
        fillId,
        fillXml,
        `Fill not found: ${fillId}`
    );
}

export function replaceBorderInStylesXml(
    stylesXml: string,
    borderId: number,
    borderXml: string
): string {
...
export function upsertNumberFormatInStylesXml(
    stylesXml: string,
    numFmtId: number,
    formatCode: string
): string {
...
export function replaceCellXfInStylesXml(
    stylesXml: string,
    styleId: number,
    xfXml: string
): string {
...
function buildStylesContainerXml(
    tagName: string,
...
function appendXmlToStylesContainer(
    ...
    const nextContainerXml = buildStylesContainerXml(
        containerTagName,
        containerTag.attributesSource,
        nextInnerXml
    );
...
function replaceIndexedXmlInStylesContainer(
    ...
    const containerTag = getRequiredXmlContainerTag(stylesXml, containerTagName, "styles.xml");
    const childTags = findXmlTags(
        containerTag.innerXml,
        getStylesContainerChildTagName(containerTagName)
    );
    ...
    const nextInnerXml =
        containerTag.innerXml.slice(0, childTag.start) +
        childXml +
        containerTag.innerXml.slice(childTag.end);
    const nextContainerXml = buildStylesContainerXml(
        containerTagName,
        containerTag.attributesSource,
        nextInnerXml
